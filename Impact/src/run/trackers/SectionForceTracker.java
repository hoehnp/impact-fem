...
/**
 * This is the section force tracker     The tracker calculates the force
 ...
 * vector to that plane 3. Calculate the nodal force component in that
 * direction 4. Summarize all nodal forces 5. Print the foce as a function of
 * time
 *
...
public class SectionForceTracker extends Tracker {
        ...
        throws IllegalArgumentException
    {
        ...
        xv.set(
            0, 0,
            ((Node) nodelist.elementAt(1)).getX_pos() -
            ((Node) nodelist.elementAt(0)).getX_pos()
        );
        xv.set(
            1, 0,
            ((Node) nodelist.elementAt(1)).getY_pos() -
            ((Node) nodelist.elementAt(0)).getY_pos()
        );
        xv.set(
            2, 0,
            ((Node) nodelist.elementAt(1)).getZ_pos() -
            ((Node) nodelist.elementAt(0)).getZ_pos()
        );

        // Next, the y-vector (will not strictly be perpendicular but it does not matter)
        yv.set(
            0, 0,
            ((Node) nodelist.elementAt(2)).getX_pos() -
            ((Node) nodelist.elementAt(0)).getX_pos()
        );
        yv.set(
            1, 0,
            ((Node) nodelist.elementAt(2)).getY_pos() -
            ((Node) nodelist.elementAt(0)).getY_pos()
        );
        yv.set(
            2, 0,
            ((Node) nodelist.elementAt(2)).getZ_pos() -
            ((Node) nodelist.elementAt(0)).getZ_pos()
        );
        ...
        for (i = 0; i < nodelist.size(); i++) {
            ...
            if (negative) {
                xv.set(
                    0, 0,
                    ((normalvector.get(0, 0) > 0)
                    ? tempnode.getX_force_component(false)
                    : tempnode.getX_force_component(true))
                );
                xv.set(
                    1, 0,
                    ((normalvector.get(1, 0) > 0)
                    ? tempnode.getY_force_component(false)
                    : tempnode.getY_force_component(true))
                );
                xv.set(
                    2, 0,
                    ((normalvector.get(2, 0) > 0)
                    ? tempnode.getZ_force_component(false)
                    : tempnode.getZ_force_component(true))
                );
            } else {
                xv.set(
                    0, 0,
        ...
        throws IllegalArgumentException
    {
        if (! Nodes_are_set) {
            throw new IllegalArgumentException(
                "No nodes defined for this section force tracker: " + number
            );
        }
        ...
        throws IOException, IllegalArgumentException
    {
        ...
        case RESULT_HEADER:
            ...
            try {
                ...
                System.out.println(
                    "Error in creating the SectionForceTracker file: " +
                    filename
                );
                throw ioe;
            }
        ...
        case RESULT:
            ...
            try {
                ...
                System.out.println(
                    "Error in writing results to the SectionForceTracker file: " +
                    filename
                );
                throw ioe;
            }
