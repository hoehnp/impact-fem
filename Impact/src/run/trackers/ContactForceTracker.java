...
/**
 * This is the contact force tracker The tracker calculates the force applied to
 * one or several contact_triangle element The force is always in the normal
...
public class ContactForceTracker extends Tracker {
	...
	public void parse_Fembic(Token[] param, int lineno,
		...
		while (i < param.length) {
			...
			if (param[i].getw().toUpperCase().equals("ELEMENTS")
					&& param[i + 1].getw().toUpperCase().equals("=")) {
				// Assume now that the element numbers are delivered in param3,
				// with the format
				...
				try {
					...
					for (j = 0; j < number_of_elements; j++) {
						elementlist.add(super.findElement(super.getNodeNumber(
			...
			if (param[i].getw().toUpperCase().equals("FILENAME")
				...
				// Neither elements nor filename is defined. Then the parameter
				// is wrong.
				throw new java.text.ParseException(
	...
	public void checkIndata() throws IllegalArgumentException {
		if (!Elements_are_set) {
			throw new IllegalArgumentException(
					"No element defined for this Contact force tracker: "
							+ number);
		}
	...
	public void print_Gid(int ctrl, double currtime) throws IOException,
		...
		case RESULT_HEADER:
			...
			try {
				...
				for (i = 0; i < elementlist.size(); i++) {
					out += (((Contact_Triangle) elementlist.elementAt(i))
							.getNumber() + ": ");
				}
				...
				System.out
						.println("Error in creating the ContactForceTracker file: "
								+ filename);
				throw ioe;
			}
		...
		case RESULT:
			...
			try {
				...
				System.out
						.println("Error in writing results to the ContactForceTracker file: "
								+ filename);
				throw ioe;
			}
