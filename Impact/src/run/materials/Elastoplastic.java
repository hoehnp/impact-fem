...
/**
 * This is an isoparametric elasto-plastic material law. 
 * The elastic Young's modulus defines the stress-strain relation up to the yield stress. 
 * Above yield stress, there are several options. The plastic behaviour can be described 
 * by the plastic modulus (EP) which defines a linear relation between the stress and 
 * effective plastic strain. This relation can also be a curve, defined by a range of 
 * stress/strain coordinates. The EP in this case has no function and can be omitted. 
 * Finally, the relation can also be dependent on the strain rate in which several 
 * stress/strain curves are defined together with a parameter setting the velocity 
 * for which each curve is representative. The stress for a certain effective strain value 
 * is detemined as a linear interpolation from these curves.
 *
