	...
	public void run() {
	    ...
	    try {
	        ...
	        while(true) {
	            ...
	            barrier.sync();    //InterruptedException will be thrown if thread interrupted
	            ...
	            for(i = 0; i < elementlist.size(); i++) {
	                ...
	                if (!temporary_element.isDeActivated()) {
	                    // Determine the element local base vectors (local coordinate system)
	                    temporary_element.updateLocalCoordinateSystem();
	                    
	                    // To reduce amount of memory in each element, loop these steps for each integration point
	                    ...
	                    /* The strains and stresses are now calculated for all integration points. Continue with the
	                     * forces. By knowing all the strains in advance, the thickness reduction
	                     * of shells can be accurately determined */
	    	...
	    	try{ barrier.sync();}  catch (Exception eb) {}
	    	System.err.println("\nWorker Interrupted: Time: "+time+"   Time step: "+timestep);
	    	sendException(e);
	        return;
	    } catch (Exception e) {
	    	// Notify listeners that an error has occurred
	    	try{ barrier.sync();}  catch (Exception eb) {}
	    	System.err.println("\nWorker Exception: Time: "+time+"   Time step: "+timestep);
	        sendException(e);
	        return;
	    }
