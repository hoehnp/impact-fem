 ...
 * both axial and torsional spring properties and is useful to model
 * spotwelds. It can also be used to model joints by releasing some degrees of
 * freedom. The elment requires a local coordinate system to be defined by
 * using other nodes as directional guidance.
 * 
...
public class Beam_Spring_2 extends Element {
    ...
    /**
     * Here, any external force contribution such as pressure is calculated,
     * but on this element there is no such feature. The only one to concider
     * is the internal contact element, should it be enabled.
     *
    ...
    public void calculateExternalForces(double currtime) {
	     ...
	     * loads, thus the "addition" is negative. The incoming loads are already
	     * in the 'stress' martrix as axial forces and moments as [fx fy fz mx my
	     * mz] where the directions are local.
	     *
	     * @param integration_point The current integration point
	     *
	     */
     ...
     * the displacement that is calculated as input to the force /
     * displacement relation used in the associated material law. The
     * displacement is calculated as [dx dy dz rx ry rz] in local directions
     * Velocity is calculated as [vx vy vz vrx vry vrz] in local directions
     *
     * @param timestep The timestep of the simulation
     * @param integration_point Which integration point of which the strain
     ...
     *
     * @param integration_point The integration point of which the stress is to
     *        be calculated
     */
        ...
        throws IllegalArgumentException
    {
        material.calculateStressOneDimensional(
            displacement, velocity, force, timestep
        );
    }
    ...
    public double checkTimestep(double current_timestep) {
        ...
        I1 = (node[0].getInertia().get(0, 0) + node[0].getInertia().get(1, 1) +
            node[0].getInertia().get(2, 2)) / 3.0;
        I2 = (node[1].getInertia().get(0, 0) + node[1].getInertia().get(1, 1) +
            node[1].getInertia().get(2, 2)) / 3.0;

        return Math.min(
            current_timestep,
            material.wavespeedOneDimensional(
                (4 * node[0].getMass() * node[1].getMass()) / (node[0].getMass() +
                node[1].getMass()), (4 * I1 * I2) / (I1 + I2)
            )
        );
    }
     ...
     * file.
     *
     * @param param The input, split up into an array of Tokens
     * @param lineno The line number.
     * @param nodelist The global nodelist
     * @param materiallist The global material list
     * @param loadlist The global load list
     *
    ...
    public String print_Gid(int ctrl, int gpn) {
        ...
        case RESULT_STRESS_LOCAL:
            ...
            /* Print a dummy value for this element, remembering the previous line index */
