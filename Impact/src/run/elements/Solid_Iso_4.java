...
public class Solid_Iso_4 extends Element {
	...
	/**
	 * This is the Solid Isoparametric Tetra element. The element can have one
	 * or eight integration points.
	 */
	public Solid_Iso_4() {
		super();
	...
	public void assembleMassMatrix() throws IllegalArgumentException {
		...
		if (total_mass <= 0) {
			throw new IllegalArgumentException(
					"Error in Solid_iso_4 (tetra) element "
							+ number
	...
	/**
	 * Calculates the D-matrix for integration point i
	 * 
	...
	/**
	 * Calculates the N-matrix for integration point
	 */
	 ...
	 * following format: f = [ fx1 fy1 fz1 fx2 fy2 ..... fz8 ]
	 */
	public void calculateNodalForces(int i, double timestep) {
		 ...
		 */
		f = B[i].transpose().times(stress[i]).times(J[i].det())
				.times(W[i] * W[i] * W[i]);
	 ...
	 * the number of the integration point currently run.
	 * 
	 * @param tstep
	 *            double
	 */
	...
	public void calculateStress(int i, double timestep) {
		...
		material[i].calculateStressThreeDimensional(strain[i], dstrain[i],
				stress[i], timestep);
	}
	 ...
	 * length since this is always the critical length.
	 * 
	...
	public double checkTimestep(double current_timestep) {
		...
		// Lower edges
		c = Math.sqrt(((node[1].getX_pos() - node[0].getX_pos()) * (node[1]
	...
	public void parse_Fembic(Token[] param, int lineno, RplVector nodelist,
		...
		while (i < param.length) {
			...
			if (param[i].getw().toUpperCase().equals("NODES")
				...
				// The nodes are set, but since this is a "disguised" hex
				// element at the moment, we need
				// to set some other nodes as well in order to collapse the
				// element.
	...
	public void checkIndata() throws IllegalArgumentException {
		...
		if (number_of_integration_points != 1)
			throw new IllegalArgumentException(
					"NIP can only be set to 1 in Solid_Iso_4 element nr"
							+ number);
	...
	public void checkIfFailed() {
		...
		if (material[0].failureStressIsSet()) {
			...
			for (int i = 0; i < number_of_integration_points; i++) {
				...
				s += 0.7071f * Math
						.sqrt(Math.pow(
								stress[i].get(0, 0) - stress[i].get(1, 0), 2)
								+ Math.pow(
										stress[i].get(1, 0)
												- stress[i].get(2, 0), 2)
								+ Math.pow(
										stress[i].get(2, 0)
												- stress[i].get(0, 0), 2)
								+ (6 * (Math.pow(stress[i].get(3, 0), 2)
										+ Math.pow(stress[i].get(4, 0), 2) + Math
											.pow(stress[i].get(5, 0), 2))));
			}
		...
		if (material[0].failureStrainIsSet()) {
			...
			for (int i = 0; i < number_of_integration_points; i++) {
				...
				e += 0.4714f * Math
						.sqrt(Math.pow(
								strain[i].get(0, 0) - strain[i].get(1, 0), 2)
								+ Math.pow(
										strain[i].get(1, 0)
												- strain[i].get(2, 0), 2)
								+ Math.pow(
										strain[i].get(2, 0)
												- strain[i].get(0, 0), 2)
								+ (1.5 * (Math.pow(strain[i].get(3, 0), 2)
										+ Math.pow(strain[i].get(4, 0), 2) + Math
											.pow(strain[i].get(5, 0), 2))));
			}
	 ...
	 * point results are to be printed.
	 * 
	 * @param ctrl
	 *            int
	 * @param gpn
	...
	public String print_Gid(int ctrl, int gpn) {
		...
		case MESH_HEADER:
			 ...
			 * is Tetrahedra but disgused as a collapsed hex element and it uses
			 * 8 nodes
			 */
	 ...
	 * (they are derived in global directions)
	 */
