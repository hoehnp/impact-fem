...
public class Shell_C0_3 extends Element {
     ...
     * loads Thus, the "addition" is negative. 
     *
     */
    public void calculateNodalForces(int integration_point, double timestep)
    {
        ...
        f.set(
            0, 0,
            0.5 * thickness * weight_factor[integration_point] * stress[integration_point].get(
                0, 0
            )
        );
        f.set(
        ...
        // For those who are studying this in detail and is wondering why the formula is 0.5*thickness and not 0.25*thickness*thickness, remember 
        ...
        moment.setMatrix(
            0, 5, 0, 0,
            Bb.transpose().times(f).plus(Bs.transpose().times(ft)).times(area)
        );
        ...
        force.set(
            8, 0,
            -(moment.get(0, 0) + moment.get(3, 0) + moment.get(6, 0)) / p.get(7,
                0
            )
        );
        force.set(
            5, 0,
            ((moment.get(1, 0) + moment.get(4, 0) + moment.get(7, 0)) -
            (p.get(6, 0) * force.get(8, 0))) / p.get(3, 0)
        );
        ...
        moment.setMatrix(
            0, 2, 0, 0,
            transposed_coordinate_system.times(moment.getMatrix(0, 2, 0, 0))
        );
        moment.setMatrix(
            3, 5, 0, 0,
            transposed_coordinate_system.times(moment.getMatrix(3, 5, 0, 0))
        );
        moment.setMatrix(
            6, 8, 0, 0,
            transposed_coordinate_system.times(moment.getMatrix(6, 8, 0, 0))
        );
        force.setMatrix(
            0, 2, 0, 0,
     ...
     * system v = [u1 v1 w1 u2 v2 w2 u3 v3 w3] p is an array of node
     * coordinates expessed in the local coordinate system p = [x1 y1 z1 x2 y2
     * z2 x3 y3 z3] a is an array of node rotation velocity expressed in the
     * local coordinate system a = [ax1 ay1 az1 ax2 ay2 az2 ax3 ay3 az3] Bm,
     * Bb and Bs are matrices of the shape function derivatives. 
     *
     * @param tstep double
     */
    public void calculateStrain(double tstep, int integration_point) {
        ...
        if (integration_point == 0) {
            ...
            rbx = (((v.get(8, 0) - v.get(2, 0)) * p.get(3, 0)) -
                ((v.get(5, 0) - v.get(2, 0)) * p.get(6, 0))) / (p.get(3, 0) * p.get(
                    7, 0
                ));
            ...
            dmxy = (Bm.get(2, 0) * v.get(0, 0)) + (Bm.get(2, 1) * v.get(1, 0)) +
                (Bm.get(2, 2) * v.get(3, 0)) + (Bm.get(2, 3) * v.get(4, 0)) +
            ...
            kappay = (Bb.get(1, 0) * a.get(0, 0)) +
                (Bb.get(1, 2) * a.get(3, 0)) + (Bb.get(1, 4) * a.get(6, 0));
            kappaxy = (Bb.get(2, 0) * a.get(0, 0)) +
                (Bb.get(2, 1) * a.get(1, 0)) + (Bb.get(2, 2) * a.get(3, 0)) +
                (Bb.get(2, 3) * a.get(4, 0)) + (Bb.get(2, 5) * a.get(7, 0));
        }
        ...
        strainrate.set(
            ...
            (Bs.get(1, 2) * a.get(3, 0)) + (Bs.get(1, 3) * a.get(4, 0)) +
            (Bs.get(1, 4) * a.get(6, 0)) + (Bs.get(1, 5) * a.get(7, 0))
        );
        ...
        strainrate.set(
            ...
            (Bs.get(0, 2) * a.get(3, 0)) + (Bs.get(0, 3) * a.get(4, 0)) +
    ...
    public double checkTimestep(double current_timestep) {
        ...
        critical_length = Math.max(
                critical_length,
                Math.sqrt(
                    ((node[0].getX_pos() - node[2].getX_pos()) * (node[0].getX_pos() -
                    node[2].getX_pos())) +
                    ((node[0].getY_pos() - node[2].getY_pos()) * (node[0].getY_pos() -
                    node[2].getY_pos())) +
                    ((node[0].getZ_pos() - node[2].getZ_pos()) * (node[0].getZ_pos() -
                    node[2].getZ_pos()))
                )
            );
        ...
        throws java.text.ParseException
    {
        ...
        while (i < param.length) {
            ...
            if (
                ...
                // The value of the element friction is in param3. Set this in the element
        ...
        if ((Contact == BASIC) || (Contact == EDGE)) {
            ...
            internal_contact_element.parse_Fembic(
        ...
        throws IllegalArgumentException
    {
        ...
        if (! T_is_set) {
            throw new IllegalArgumentException(
                "No Thickness (T) defined for Shell_C0_3 element nr" + number
            );
        }
    ...
    public String print_Gid(int ctrl, int gpn) {
        ...
        case MESH_HEADER:
             ...
             * In this case, the type of element is Triangular and it uses 3
               nodes
             */
            out = new String(
                    "MESH \"MeshType" + type +
    ...
    private void calculateLocalVariables() {
        ...
        p.setMatrix(
            3, 5, 0, 0, p.getMatrix(3, 5, 0, 0).minus(p.getMatrix(0, 2, 0, 0))
        );
        p.setMatrix(
            6, 8, 0, 0, p.getMatrix(6, 8, 0, 0).minus(p.getMatrix(0, 2, 0, 0))
        );
        p.setMatrix(
            0, 2, 0, 0, p.getMatrix(0, 2, 0, 0).minus(p.getMatrix(0, 2, 0, 0))
        );
    ...
    public void deActivate() {
        super.deActivate();
        if (internal_contact_element != null) internal_contact_element.deActivate();
