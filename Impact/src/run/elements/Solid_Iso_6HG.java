...
public class Solid_Iso_6HG extends Element {
        ...
        throws IllegalArgumentException
    {
        ...
        if (total_mass <= 0) {
            throw new IllegalArgumentException(
                "Error in Solid Hexahedron Element " + number +
    ...
    public void calculateNodalForces(int dummy, double dt) {
        ...
        GS=GB; // I don't know if this will work, but...
        for(i=0; i<8; i++) {
            for(j=0; i<3; j++) {
                A=A+BB[i][j]*BB[i][j];
                S0=BB[i][j]/vol;
        ...
        // Shear modulus... Need to be used here... Please help in doing so...
        G= (material[0].getYoungsModulus())/(2*(1+(material[0].getNu())));
        
        // Global time step, dt,  is needed here... How to get it?
        // AHR is an user defined parameter to control HG... we need it together 
        // with the material properties...
        A= dt*AHR*(material[0].getYoungsModulus()+G+G)*A/(24*vol);
        for(i=0; i<3; i++) {
            for(k=0; k<4; k++) {
                S0=0.;
                for( j=0; j<8; j++) {
                ...
                elemp[i][k]=S0;
                for( j=0; j<8; j++) {
                    // We need to add the hour glass stabilization force to
                    // the global force vector... The indices are a little confusing
                    // but it works...
                    //    i is the direction (x, y or z)
                    //    j is the local node number
                    //    k is the assiciated HG mode
        ...
        for (i=0; i<8; i++)
            node[i].addHourglassForce(hforce[i]);
*/      
    }
        ...
        throws java.text.ParseException
    {
        ...
        while (i < param.length) {
            ...
            if (
                ...
                // The cross section area of the element is defined
                if (
                    param[i].getw().toUpperCase().equals("AHR") &&
                    param[i + 1].getw().toUpperCase().equals("=")
                ) {
                    // The value of the cross section area is in param4. Set this in the element
                    AHR = param[i + 2].getn();
        ...
        throws IllegalArgumentException
    {
        ...
        if (! AHR_is_set) {
            throw new IllegalArgumentException(
                "No Hourglass parameter AHR set for Solid_Iso_6HG element nr" + number
            );
        }
    }
    ...
    public String print_Gid(int ctrl, int gpn) {
        ...
        case MESH_HEADER:
             ...
             * In this case, the type of element is Hexahedra (6-sided solid element) and it uses 8 nodes
             */
            out = new String(
                    "MESH \"MeshType" + type +
