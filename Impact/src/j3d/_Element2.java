...
public class _Element2 extends _Element implements Serializable{
  ...
  public _Element2(_Node n1, _Node n2, int t, Material m, float d){
    msh_type=t;
    if(t==Canvas3D.MESH_Dummy_2)msh_name = "Dummy_2";
    else if(t==Canvas3D.MESH_Beam_2)msh_name = "Beam_2";
    else if(t==Canvas3D.MESH_Rod_2)msh_name = "Rod_2";
    else if(t==Canvas3D.MESH_Contact_Line)msh_name = "Contact_Line";
    node1=n1; node2=n2; material=m; diameter=d;
  }
  ...
  public String writeObject() {
      ...
      st+=" \tnodes = ["+node1.get_Id()+","+node2.get_Id()+"] \tD = "+diameter;
      if(factor!=null) st+=" \tFactor = "+factor;
      if(friction!=null) st+=" \tFriction = "+friction;
      
      if (msh_type == Canvas3D.MESH_Contact_Line) return st;
      
      if(material != null) st+="\tMaterial = " +material.name;
      if(contact!=null && contact.compareToIgnoreCase("OFF")==0) st+=" \tContact = OFF";
  ...
  public Object[] get_Array(Canvas3D j3d){
    ...
    if(material==null)  arr[k++]=new shpLine(node1.x,node1.y,node1.z,node2.x,node2.y,node2.z,selected ? Canvas3D.SELECTCOLOR : contur);
    else arr[k++]=new shpLine(node1.x,node1.y,node1.z,node2.x,node2.y,node2.z,selected ? Canvas3D.SELECTCOLOR : material.color);

    if(j3d.getSHOW_ID_ELEMENT()){
      arr[k++]=new shpOrientedText(get_Id(),(node1.x+node2.x)/2,(node1.y+node2.y)/2,(node1.z+node2.z)/2,contur);
    }
    if(j3d.getSHOW_ID_LOADS() && load!=null)arr[k++]=new shpOrientedText(load.name,(node1.x+node2.x)/2,(node1.y+node2.y)/2,(node1.z+node2.z)/2,load.color);
  ...
  void jButton1_actionPerformed(ActionEvent e) {
      ...
      try{
          ...
          if((node1_tmp==null || node2_tmp==null) && (node1==null || node2==null))return;
          float d = java.lang.Float.parseFloat(tfd.getText());
          java.lang.Float fact,frict;
          fact=tffact.getText().trim().length()==0?null:new java.lang.Float(tffact.getText());
          frict=tffrict.getText().trim().length()==0?null:new java.lang.Float(tffrict.getText());
          if(node1_tmp!=null)node1=node1_tmp;
          if(node2_tmp!=null)node2=node2_tmp;
          diameter=d;
          factor=fact;
          friction=frict;
          contact=(cb_ctype.getSelectedItem()+"").length()==0?null:cb_ctype.getSelectedItem()+"";
          msh_name=cb_type.getSelectedItem()+"";
          if(cb_type.getSelectedIndex()==0) msh_type = Canvas3D.MESH_Rod_2;
          else if(cb_type.getSelectedIndex()==1) msh_type = Canvas3D.MESH_Beam_2;
          else if(cb_type.getSelectedIndex()==2) msh_type = Canvas3D.MESH_Contact_Line;
