...
public class _NurbSurface extends _Geometry implements Serializable{
  ...
  public String writeObject() {
    ...
    if (contact != null) st += " CONTACT = " + contact;
...
/*
  private void writeObject(ObjectOutputStream out)  throws IOException{
    ...
    out.writeObject(uknots);
    out.writeObject(vknots);
    out.writeInt(udiv);
    out.writeInt(vdiv);
    out.writeObject(color);
    out.writeInt(geometry_type);
    out.writeObject(msh_name);
    out.writeInt(msh_type);
    out.writeObject(type);
    out.writeFloat(thickness);
    out.writeObject(mesh_div);
    out.writeFloat(mesh_size);
    out.writeObject(factor);
    out.writeObject(friction);
    out.writeObject(contact);
    out.writeObject(material);
    out.writeObject(material_index);
    out.writeObject(geomUDB);
    out.writeObject(geomVDB);
    out.writeObject(geomBDB);
...
*/
  ...
  public void readObject(String st) {
    ...
    uknots = (float[])in.readObject();
    vknots = (float[])in.readObject();
    udiv = in.readInt();
    vdiv = in.readInt();
    ...
    geomUDB = (Vector)in.readObject();
    geomVDB = (Vector)in.readObject();
    geomBDB = (Vector)in.readObject();
    nodesDB = (XVector)in.readObject();
...
public void reset(boolean do_mesh){
  ...
  /**
   * This method generates the points which makes up the curve. It also calculates the length of the surface edges.
   *
  ...
  public void generate(int udiv, int vdiv) {
    ...
    // Generate two middle curves and a boundary to repesent the surface geometry

    this.generateRefinedCurve(geomUDB, numUControlPoints, uorder, uknots, numVControlPoints, vorder, vknots, controlPoints,-1f,0.5f,true);
    this.generateRefinedCurve(geomVDB, numUControlPoints, uorder, uknots, numVControlPoints, vorder, vknots, controlPoints, 0.5f,-1f,true);

    edgelength[0] = this.generateRefinedCurve(geomBDB, numUControlPoints, uorder, uknots, numVControlPoints, vorder, vknots, controlPoints, -1f, 0.0f,true);
    edgelength[1] = this.generateRefinedCurve(geomBDB, numUControlPoints, uorder, uknots, numVControlPoints, vorder, vknots, controlPoints, 1.0f, -1f,true);
    edgelength[2] = this.generateRefinedCurve(geomBDB, numUControlPoints, uorder, uknots, numVControlPoints, vorder, vknots, controlPoints, -1f, 1.0f,false);
    edgelength[3] = this.generateRefinedCurve(geomBDB, numUControlPoints, uorder, uknots, numVControlPoints, vorder, vknots, controlPoints, 0.0f, -1f,false);
  }

  private float generateRefinedCurve(Vector db, int n, int p, float[] U, int m, int q, float[] V, _CtrlPoint[][] P, float u, float v, boolean positive)  {
      ...
      for (int i=order+1; i < numKnots-order; i++)
          if (((Float)tmp.lastElement()).floatValue() != (u < 0 ? U[i] : V[i])) tmp.add(new Float(u < 0 ? U[i] : V[i]));
      ...
      for (int i=0; i<tmp.size()-1; i++) {
          ...
          s = lib3D.surfacePoint(n, p, U, m, q, V, P, u < 0 ? ts : u, v < 0 ? ts : v);
          e = lib3D.surfacePoint(n, p, U, m, q, V, P, u < 0 ? te : u, v < 0 ? te : v);
          a = lib3D.surfacePoint(n, p, U, m, q, V, P, u < 0 ? (ts+te)/2f : u, v < 0 ? (ts+te)/2f : v);
      ...
      if (positive)
          for (int i=0; i<tmp.size(); i++) {
              e = lib3D.surfacePoint(n, p, U, m, q, V, P, u < 0 ? ((Float)tmp.elementAt(i)).floatValue() : u, v < 0 ? ((Float)tmp.elementAt(i)).floatValue() : v);
              db.add(e);
              if (s != null) distance += e.distance(s);
              s = e;
          } else
  ...
  /**
   * This method generates the nodes and elements which makes up the surface mesh.
   * Note: before calling this method, make sure you have set the following
   * knots 			- the knot vectors
   * controlPoints 	- The controlPoint array
   * order			- Curve segment degrees
   *
   * Each edge has a number of divisions (div[0] -> div[3])
   *                                                     [2]
   * They are defined in the following way          +------------+
   * div[0] = divisions of curve u=0-1, v=0         |            |
   * div[1] = divisions of curve u=1, v=0-1     [3] |            | [1]
   * div[2] = divisions of curve u=0-1, v=1         |            |
   * div[3] = divisions of curve u=0, v=0-1         +------------+
   *                                                     [0]
   *
   ...
   *
   * For now, we settle for using only constant divisions defined by div[0] and div[1]
   *
   ...
   * 
   * @param type                    Type of mesh. Can be triangle or mixed quad and tria
   * @param numUControlPoints       Number of controlpoints for the surface in U direction
   * @param numVControlPoints       Number of controlpoints for the surface in V direction
   * 
   * Notice that the number of boundary nodes are set with the mesh_div[] variable in a counter clockwise direction.
   * The distribution of the element sizes are controlled inside the Circle object by calculating a quota. An element
   * is concidered too large if the quota is greater than 1.
   *
   */
  
  
  private void mesh_unstructured(int type, int numUControlPoints, int numVControlPoints) {
      _Point np;
      _Node nn;
      _Element3 nf;
      ...
      if (msh_type == Canvas3D.MESH_Dummy_4) return;
      
      // Generate mesh nodes
      for (i=1; i<=mesh_div[0]; i++) {
          np = lib3D.surfacePoint(numUControlPoints, uorder, uknots, numVControlPoints, vorder, vknots, controlPoints, (float)i/(float)mesh_div[0], 0);
          nn = new _Node(np.x,np.y,np.z);
          nn.set_Id(Id + "." + nodid++);
          nn.setMaster_type(Canvas3D.SURFACE);
          nn.setShow(show);
          tnodesDB.add(nn);
          pnts.add(new Point2D.Double((double)i/mesh_div[0], 0.0 + Math.random()*1E-9));
      }

      for (i=1; i<=mesh_div[1]; i++) {
          np = lib3D.surfacePoint(numUControlPoints, uorder, uknots, numVControlPoints, vorder, vknots, controlPoints, 1, (float)i/(float)mesh_div[1]);
          nn = new _Node(np.x,np.y,np.z);
          nn.set_Id(Id + "." + nodid++);
          nn.setMaster_type(Canvas3D.SURFACE);
          nn.setShow(show);
          tnodesDB.add(nn);
          pnts.add(new Point2D.Double(1.0 - Math.random()*1E-9,(double)i/mesh_div[1]));
      }

      for (i=1; i<=mesh_div[2]; i++) {
          np = lib3D.surfacePoint(numUControlPoints, uorder, uknots, numVControlPoints, vorder, vknots, controlPoints, 1.0f - (float)i/(float)mesh_div[2], 1);
          nn = new _Node(np.x,np.y,np.z);
          nn.set_Id(Id + "." + nodid++);
          nn.setMaster_type(Canvas3D.SURFACE);
          nn.setShow(show);
          tnodesDB.add(nn);
          pnts.add(new Point2D.Double(1.0 - (double)i/mesh_div[2], 1.0 - Math.random()*1E-9));
      }

      for (i=1; i<=mesh_div[3]; i++) {
          np = lib3D.surfacePoint(numUControlPoints, uorder, uknots, numVControlPoints, vorder, vknots, controlPoints, 0, 1.0f - (float)i/(float)mesh_div[3]);
          nn = new _Node(np.x,np.y,np.z);
          nn.set_Id(Id + "." + nodid++);
          nn.setMaster_type(Canvas3D.SURFACE);
          nn.setShow(show);
          tnodesDB.add(nn);
          pnts.add(new Point2D.Double(0.0 + Math.random()*1E-9,1.0 - (double)i/mesh_div[3]));
      }
      ...
      for (i=0; i<n-2; i++)
          for (j=i+1; j<n-1; j++)
              if (j != i)
                  for (k=j+1; k<n; k++)
                      if (k != i && k != j && Math.abs(lib2D.crossProduct((Point2D.Double)pnts.elementAt(i),(Point2D.Double)pnts.elementAt(j),(Point2D.Double)pnts.elementAt(k))) > 1E-9) {
                          c.circumCircle((Point2D.Double)pnts.elementAt(i),(Point2D.Double)pnts.elementAt(j),(Point2D.Double)pnts.elementAt(k));
                          free = true;

                          for (l=0; l<n; l++)
                              if (l != i && l != j && l != k)
                                  if (c.inside((Point2D.Double)pnts.elementAt(l))) {
                                      free = false;
                                      break;
                                  }

                          if (free) {
                              nf = new _Element3((_Node)tnodesDB.elementAt(i),(_Node)tnodesDB.elementAt(j),(_Node)tnodesDB.elementAt(k),Canvas3D.MESH_Shell_C0_3,material,thickness);
                              nf.set_Id(Id + "." + elid++);
                              nf.setShow(show);
      ...
      while (!badElements.isEmpty()) {
          ...
          if (i != -1) {
              ...
              np = lib3D.surfacePoint(numUControlPoints, uorder, uknots, numVControlPoints, vorder, vknots, controlPoints, (float)c.c.x, (float)c.c.y);
              nn = new _Node(np.x,np.y,np.z);
              nn.set_Id(Id + "." + nodid++);
              nn.setMaster_type(Canvas3D.SURFACE);
              nn.setShow(show);
              ...
              // Add the nodes and points of these elements to a temporary vector
              ...
              for (i=0; i<n-1; i++) {
                  c = new Circle();
                  c.circumCircle(tp,(Point2D.Double)tempPoints.elementAt(i),(Point2D.Double)tempPoints.elementAt(i+1));
                  nf = new _Element3(nn,(_Node)tempNodes.elementAt(i),(_Node)tempNodes.elementAt(i+1),Canvas3D.MESH_Shell_C0_3,material,thickness);
                  nf.set_Id(Id + "." + elid++);
                  nf.setShow(show);
      ...
      for (i=telementsDB.size()-1; i>=0; i--) {
          ...
          if (nf.getMinLength() < 1e-5) // This is an element which can occur at edges for triangular surfaces
...
private void mesh_structured(int type, int numUControlPoints, int numVControlPoints) {
    _Point np;
    _Node nn;
    _Element4 ne;
    _Element3 nf;
    if (msh_type != Canvas3D.MESH_Dummy_4) {
          ...
          for (int i=0; i<=mesh_div[0]; i++) {
              for (int j=0; j<=mesh_div[1]; j++){
                  np = lib3D.surfacePoint(numUControlPoints, uorder, uknots, numVControlPoints, vorder, vknots, controlPoints, (float)i/(float)mesh_div[0], (float)j/(float)mesh_div[1]);
                  nn = new _Node(np.x,np.y,np.z);
                  nn.set_Id(Id + "." + i + "." + j);
                  nn.setMaster_type(Canvas3D.SURFACE);
                  nn.setShow(show);
              ...
              if (i < mesh_div[0]) nodesDB.addRow();
          }
          ...
          for (int i=0; i<mesh_div[0]; i++) {
              for (int j=0; j<mesh_div[1]; j++){
                  ...
                  case Canvas3D.MESH_Shell_BT_4:
                      if (this.areClose((_Node)nodesDB.elementAt(i,j),(_Node)nodesDB.elementAt(i+1,j))) {
                          nf = new _Element3((_Node)nodesDB.elementAt(i,j),(_Node)nodesDB.elementAt(i+1,j+1),(_Node)nodesDB.elementAt(i,j+1),Canvas3D.MESH_Shell_C0_3,material,thickness);
                          nf.set_Id(Id + "." + i +"." + j + ".1");
                          nf.setShow(show);
                          elementsDB.add(nf);
                      }
                      else
                          if (this.areClose((_Node)nodesDB.elementAt(i+1,j),(_Node)nodesDB.elementAt(i+1,j+1))) {
                              nf = new _Element3((_Node)nodesDB.elementAt(i,j),(_Node)nodesDB.elementAt(i+1,j),(_Node)nodesDB.elementAt(i,j+1),Canvas3D.MESH_Shell_C0_3,material,thickness);
                              nf.set_Id(Id + "." + i +"." + j + ".1");
                              nf.setShow(show);
                              elementsDB.add(nf);
                          }
                          else
                              if (this.areClose((_Node)nodesDB.elementAt(i+1,j+1),(_Node)nodesDB.elementAt(i,j+1))) {
                                  nf = new _Element3((_Node)nodesDB.elementAt(i,j),(_Node)nodesDB.elementAt(i+1,j),(_Node)nodesDB.elementAt(i,j+1),Canvas3D.MESH_Shell_C0_3,material,thickness);
                                  nf.set_Id(Id + "." + i +"." + j + ".1");
                                  nf.setShow(show);
                                  elementsDB.add(nf);
                                  ...
                                  if (this.areClose((_Node)nodesDB.elementAt(i,j),(_Node)nodesDB.elementAt(i,j+1))) {
                                      nf = new _Element3((_Node)nodesDB.elementAt(i,j),(_Node)nodesDB.elementAt(i+1,j),(_Node)nodesDB.elementAt(i+1,j+1),Canvas3D.MESH_Shell_C0_3,material,thickness);
                                      nf.set_Id(Id + "." + i +"." + j + ".1");
                                      nf.setShow(show);
                                      elementsDB.add(nf);
                                  }
                                  else {
                                      ne = new _Element4((_Node)nodesDB.elementAt(i,j),(_Node)nodesDB.elementAt(i+1,j),(_Node)nodesDB.elementAt(i+1,j+1),(_Node)nodesDB.elementAt(i,j+1),type,material,thickness);
                                      ne.set_Id(Id + "." + i +"." + j);
                                      ne.setShow(show);
                                      elementsDB.add(ne);
                                  }
                  ...
                  case Canvas3D.MESH_Shell_C0_3:
                      nf = new _Element3((_Node)nodesDB.elementAt(i,j),(_Node)nodesDB.elementAt(i+1,j),(_Node)nodesDB.elementAt(i+1,j+1),type,material,thickness);
                      nf.set_Id(Id + "." + i +"." + j + ".1");
                      nf.setShow(show);
                      elementsDB.add(nf);
                      nf = new _Element3((_Node)nodesDB.elementAt(i+1,j+1),(_Node)nodesDB.elementAt(i,j+1),(_Node)nodesDB.elementAt(i,j),type,material,thickness);
                      nf.set_Id(Id + "." + i +"." + j + ".2");
                      nf.setShow(show);
                      elementsDB.add(nf);
                      break;
              ...
              if (i < mesh_div[0] - 1) elementsDB.addRow();
          }
      }
}
  ...
  public String toString(){ return "Nurb Surface ID="+Id;}
  ...
  protected void update(ActionEvent e) {
      ...
      if(mesh_type_index==1) msh_type = Canvas3D.MESH_Shell_BT_4;
      if(mesh_type_index==2) msh_type = Canvas3D.MESH_Shell_C0_3;
      if(mesh_type_index==3) msh_type = Canvas3D.MESH_Contact_Triangle;
      if(mesh_type_index==0) msh_type = Canvas3D.MESH_Dummy_4;
  ...
  public _Object duplicate(Canvas3D out, boolean add){
   ...
   public void insertKnots(float[] ins, boolean udir) {
      ...
      lib3D.refineKnotVectSurface((udir?uorder:vorder), (udir?uknots:vknots), controlPoints, ins, newknot, newcontrolPoints);
...
public void elevate(int times, boolean udir) {
      ...
      _CtrlPoint[] newcontrolPoints = new _CtrlPoint[ctrlPoints.length*(times+1)];
      ...
      for (int strip = 0; strip < nr_of_strips; strip++) {
          ...
          // Elevate it
          cl = lib3D.degreeElevateCurve(order, knots, ctrlPoints, times, newknots, newcontrolPoints);
...
public Vector3D getCenter() {
       ...
       _Point s = lib3D.surfacePoint(numUControlPoints, uorder, uknots, numVControlPoints, vorder, vknots, controlPoints, 0.5f, 0.5f);
...
public boolean isPickPoint(Rectangle2D r, boolean shw, boolean ogl) {
   ...
   private boolean areClose(_Node n1, _Node n2) {
       ...
       if (d > J3D.getNODE_MERGE_TOLERANCE()*J3D.getNODE_MERGE_TOLERANCE()) return false;
...
public _Object[] getBorderObjects() {
      ...
      c[0] = new _NurbCurve(controlPoints[0],vknots,vorder,5,Canvas3D.MESH_Dummy_2,1.0f,null,J3D);

      for (int i=0; i<controlPoints.length; i++)
          tmp[i] = controlPoints[i][0];
      c[1] = new _NurbCurve(tmp,uknots,uorder,5,Canvas3D.MESH_Dummy_2,1.0f,null, J3D);

      c[2] = new _NurbCurve(controlPoints[controlPoints.length-1],vknots,vorder,5,Canvas3D.MESH_Dummy_2,1.0f,null,J3D);

      for (int i=0; i<controlPoints.length; i++)
          tmp[i] = controlPoints[i][controlPoints[0].length-1];
      c[3] = new _NurbCurve(tmp,uknots,uorder,5,Canvas3D.MESH_Dummy_2,1.0f,null,J3D);
...
public _Point getPointAt(float u, float v) {
    ...
    int numVControlPoints = controlPoints[0].length;
      ...
      return lib3D.surfacePoint(numUControlPoints, uorder, uknots, numVControlPoints, vorder, vknots, controlPoints,u,v);
  }
