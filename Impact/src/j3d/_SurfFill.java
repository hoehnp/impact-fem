...
public class _SurfFill extends _NurbSurface implements Serializable{
  ...
  private static Regex r_curve = new Regex("(?i)CURVES *= *\\[ *([0-9, ]+) *\\]");
...
//        tcurve[i] = (_NurbCurve)curve[i].copy();
    ...
    if (!lib3D.isClose(pnt[1],tmp[0],J3D.getGeometricTolerance()) && !lib3D.isClose(pnt[1],tmp[1], J3D.getGeometricTolerance())) {
    ...
    for (i=1; i<4; i++) {
        ...
        if (!lib3D.isClose(pnt[1],tmp[0],J3D.getGeometricTolerance()) && !lib3D.isClose(pnt[1],tmp[1],J3D.getGeometricTolerance())) 
            throw new IllegalArgumentException("Illegal input of curves. Must be in a circle");
    ...
    A = new _SurfRul(curve[0],curve[2],Canvas3D.MESH_Dummy_4,thickness,material, J3D);
    
    // 2. Generate a lofted surface B
    B = new _SurfRul(curve[1],curve[3],Canvas3D.MESH_Dummy_4,thickness,material, J3D);
    ...
    // 11. S(u,v) = S1(u,v) + S2(u,v) - T(u,v)  (do this on the control points)
    //     Note the reversed index on surface B since the orientation is different
    li = A.controlPoints.length;
    lj = A.controlPoints[0].length;
    controlPoints = new _CtrlPoint[li][lj];
    for (i = 0; i < li; i++)
    	for (j=0; j < lj; j++)
    		controlPoints[i][j] = new _CtrlPoint( A.controlPoints[i][j].x + B.controlPoints[j][i].x - C.controlPoints[i][j].x
    				                            , A.controlPoints[i][j].y + B.controlPoints[j][i].y - C.controlPoints[i][j].y
    				                            , A.controlPoints[i][j].z + B.controlPoints[j][i].z - C.controlPoints[i][j].z
    				                            , A.controlPoints[i][j].w * B.controlPoints[j][i].w * C.controlPoints[i][j].w
    				                            , Color.GRAY);
...
public String toString(){ return "FillSurface ID="+Id;}
  ...
  public JPanel getEditPanel(Canvas3D j3d, PreProcessor pp){
      ...
      jTextField1 = new JTextField((curve[0] != null ? curve[0].toString() : "")+"",3);
      jTextField2 = new JTextField((curve[1] != null ? curve[1].toString() : "")+"",3);
      jTextField3 = new JTextField((curve[2] != null ? curve[2].toString() : "")+"",3);
      jTextField4 = new JTextField((curve[3] != null ? curve[3].toString() : "")+"",3);
  ...
  void jButton1_actionPerformed(ActionEvent e) {
      _SurfFill tmp = this;
        ...
        if(curve[0]==null || curve[1]==null || curve[2]==null || curve[3]==null) return;
  ...
  void jButton2_actionPerformed(ActionEvent e) {
    ...
    public void replaceObjectWith(_Object o, _Object replacement) {
        ...
        for (int i=0; i<curve.length; i++)
            if (curve[i] == o) curve[i] = (_NurbCurve)replacement;
