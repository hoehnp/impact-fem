...
public class shpXYZ implements shp{
  ...
  public void paint(Graphics2D g, Canvas3D j3d){
    ...
    TextLayout tx = new TextLayout("X", g.getFont(), g.getFontRenderContext());
    tx.draw(g,xx,xy);
    TextLayout ty = new TextLayout("Y", g.getFont(), g.getFontRenderContext());
    ty.draw(g,yx,yy);
    TextLayout tz = new TextLayout("Z", g.getFont(), g.getFontRenderContext());
    tz.draw(g,zx,zy);
  }
