...
public class _SurfRev extends _NurbSurface implements Serializable{
  ...
  private static Regex r_start = new Regex("(?i)AXIS_STARTPOINT *= *([0-9]+) *");
  private static Regex r_end = new Regex("(?i)AXIS_ENDPOINT *= *([0-9]+) *");
  private static Regex r_angle = new Regex("(?i)ANGLE *= *([-+]?[0-9]+[.]?[0-9]*(?:[eE][-+]?[0-9]+)?)");
  private static Regex r_meshsize = new Regex("(?i)MESHSIZE *= *([-+]?[0-9]+[.]?[0-9]*(?:[eE][-+]?[0-9]+)?)");
  ...
  public void readObject(String st, Hashtable geo) {
      ...
      if (r_start.search(st)) s_axis = (_Point)geo.get(r_start.stringMatched(1));
      if (r_end.search(st)) e_axis = (_Point)geo.get(r_end.stringMatched(1));
      if (r_angle.search(st)) angle = Float.parseFloat(r_angle.stringMatched(1));
      if (r_meshsize.search(st)) mesh_size = Float.parseFloat(r_meshsize.stringMatched(1));
      super.readObject(st);
    }
  ...
  public void reset(boolean do_mesh) {
      ...
      Temp1 = new Vector3D();
        ...
        if (angle <= 90) narcs = 1;
        else if (angle <= 180) narcs = 2;
        else if (angle <= 270) narcs = 3;
        else narcs = 4;
        ...
        for (j=0; j < m; j++)
            for (i=0; i < n; i++)  {
                controlPoints[i][j] = new _CtrlPoint(Pij[i][j].x, Pij[i][j].y, Pij[i][j].z, wij[i][j], Color.GRAY);
  ...
  public String toString(){ return "RevSurface ID="+Id;}
  ...
  public JPanel getEditPanel(Canvas3D j3d, PreProcessor pp){
      ...
      jTextField2 = new JTextField((s_axis != null ? s_axis.toString() : "")+"",3);
      jTextField3 = new JTextField((e_axis != null ? e_axis.toString() : "")+"",3);
  ...
  void jButton1_actionPerformed(ActionEvent e) {
      _SurfRev tmp = this;
        ...
        if(profile==null || s_axis==null || e_axis==null) return;
  ...
  void jButton2_actionPerformed(ActionEvent e) {
    ...
    public void replaceObjectWith(_Object o, _Object replacement) {
        ...
        if (o instanceof _Point && replacement instanceof _Point) {
            if (s_axis == o) s_axis = (_Point)replacement;
            if (e_axis == o) e_axis = (_Point)replacement;
        }        
    }    
