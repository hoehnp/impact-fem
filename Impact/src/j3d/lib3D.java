...
public class lib3D implements Serializable{
  ...
  /**
 * Caluates the intersection between a <code>_Line</code>
 * object and a <code>_Triangle</code>.
 *
 * @param line The segment to use in the intersection test.
 * @param triangle An array holding the triangle data.
 * @return point if the <code>_Line</code> intersects the <code>_Triangle</code>,
 *  <code>null</code> if the <code>_Line</code> does not intersect the <code>_Triangle</code>.
 */
public static float[] LineAndTriangle( _Element2Post line, _Element3Post triangle) {
...
/**
 * Caluates the intersection between a line (l1,l2)
 * object and a triangle (t1,t2,t3).
 *
 * @param l1, l2 The segment to use in the intersection test.
 * @param t1, t2 t3 An array holding the triangle data.
 * @return point if the line intersects the triangle,
 *  <code>null</code> if the line does not intersect the object.
 */
public static float[] IntersectLineAndTriangle(_Point t1, _Point t2, _Point t3, _Point l1, _Point l2) {
  _Point pnts[] = new _Point[3];
  pnts[0] = t1;
  pnts[1] = t2;
  pnts[2] = t3;
...
private static float[] IntersectLineAndPoly(_Point coordinates[],  _Point segment[]) {
  ...
  if(pNrmDotrDir== 0.0)  return null;
  tempV3d.set(start);
  ...
  public static float[] PL3T(_Point p0, _Point p1, _Point p2){
    ...
    p[1]=a2-a6;
    p[2]=a3-a4;
    p[3]=p0.y*a6+p0.x*a5+p0.z*a4-p0.z*a3-p0.y*a2-p0.x*a1;
    return p;
  }

  public static float[] PL3T_LINE(_Point p1, _Point p2, _Point p3, _Point t1, _Point t2){
    float[] ABCD = PL3T(p1,p2,p3);
    float m = t2.x-t1.x;
    float n = t2.y-t1.y;
    float p = t2.z-t1.z;
    if(ABCD[0]*m+ABCD[1]*n+ABCD[2]*p==0 || java.lang.Float.isInfinite(ABCD[0]*m+ABCD[1]*n+ABCD[2]*p)) return null;
    float t = (-ABCD[0]*t1.x-ABCD[1]*t1.y-ABCD[2]*t1.z-ABCD[3])/(ABCD[0]*m+ABCD[1]*n+ABCD[2]*p);
  ...
  public static boolean compare_points(_Point p1, _Point p2, double e){
    return (p1.x==p2.x && p1.y==p2.y && p1.z==p2.z) || (compare_double(p1.x,p2.x,e) && compare_double(p1.y,p2.y,e) && compare_double(p1.z,p2.z,e));
  }
  ...
  /**
   * Generates a _Point on a NURBS surface determined by the internal curve coordinates u & v
   *
   * @param n		Number of control points in U direction
   * @param p		Curve segment degree in U direction
   * @param U		Knot vector U
   * @param m		Number of control points in V direction
   * @param q		Curve segment degree in V direction
   * @param V		Knot vector V
   * @param Pw		Array of Control points (weighted values used)
   * @param u		Internal surface coordinate for the generated point in u direction
   * @param v		Internal surface coordinate for the generated point in v direction
   * @return		A _Point in 3D space on the surface
   */

  public static _Point surfacePoint(int n, int p, float[] U, int m, int q, float[] V, _CtrlPoint[][] Pw, float u, float v) {
      int uspan, vspan, k,l;
   ...
   * @param p		Curve segment degree
   * @param U		Knot vector
   * @param Pw		Vector of Control points (weighted values used)
   * @param u		Internal curve coordinate for the generated point
   * @return		A _Point in 3D space on the curve
   */

  public static _Point curvePoint(int n, int p, float[] U, _CtrlPoint[] Pw, float u) {
      int span,i;
...
*
* @param n		set to nr_of_controlpoints -1
* @param p		Curve degree
* @param u		position on Nurb (usually between 0 and 1)
* @param U		Knot vector
...
*/
   ...
   * @param u		Curve internal coordinate
   * @param p		Curve degree
   * @param U		Knot vector
  ...
  /**
   * This function calculates all non-zero basis functions and their derivatives
   * @param i		Basis function index
   * @param u		Point internal u coordinate
   * @param p		Curve degree
   * @param n		number of controlpoints
   * @param U		Knot vector
   * @return       ders basis functions
   */
  public static float[][] dersBasisFuns(int i, float u, int p, int n, float[] U) {
  ...
  /**
   * This function computes the derivatives for a nurb curve. All derivatives from
   * 0 to d are calculated at point u on the curve. They are delivered as _CtrlPoints.
   *
   * @param n		Number of controlpoints
   * @param p		Curve segment degree (order)
   * @param U		knot vector
   * @param P		Vector of controlpoints
   * @param u		position on curve which to calculate derivatives (usually 0 <= u <= 1)
   * @param d		highest derivative to calculate
   * @return		An array of derivatives from 0 to d, at point u
   */

  public static _CtrlPoint[] curveDerivsAlg1(int n, int p, float[] U,  _CtrlPoint[] P, float u, int d) {
      int du,k,span,j;
      _CtrlPoint[] CK;
      float[][] nders;
  ...
  /**
   * Calculates the curve derivative for a rational B-spline as an array
   * of _CtrlPoints where [0] is the curve point in space, [1] is the first
   * derivative at that point and so on up to [d]
   *
   * @param CKa		Derivatives for a normal B-spline as given by curveDerivsAlg1()
   * @param d		Max number of the derivative requested
...
return a[n];
  ...
  private static float bin(int n, int k) {
      return (float)Math.floor(0.5+Math.exp(_factln(n)-_factln(k)-_factln(n-k)));
      }
   ...
   */
  public static Vector3D pointToLine(Vector3D start, Vector3D direction, Vector3D point) {
 ...
 //*******************************************************************************
 // Other functions which perform various operations on Nurb curves and surfaces *
 //*******************************************************************************


  /**
   * Global curve interpolation of controlpoints in order to make the curve pass through given points
   * 
   * @param Q   Points curve should be fitted to
   * @param p   Curve degree 
   * @param U   Resulting knot vector
   * @param P   Resulting control point vector
   */
  
  public static void globalCurveInterp(_Point[] Q, int p, float[] U, _CtrlPoint[] P) {
      ...
      if (P.length != Q.length)
          throw new IllegalArgumentException("Points and Control Points must be of equal number");
  ...
  /**
   * Creates one or more points which is the normal projection of the p_original point onto the c_target curve
   *
   * @param p_original	Point to be projected
   * @param c_target		Curve the point is to be projected onto
   * @return				The projected point or points
   */
  public static _Point[] projectPointOntoCurve(_Point p_original, _NurbCurve c_target, float gtol) {
      float[]  u = {0.5f,0.0f};
      float[] du = {0.1f,0.1f};
      _Point[] p;
      _Point p1, p2;
      ...
      if (p_original.vx != 0.0f || p_original.vy != 0.0f || p_original.vz != 0.0f) {

          // Project point along the point vector
          distfunction = new DirectedDistancePointToCurveFunction(p_original,c_target);

          try {
              ConstrainedDownhillSimplex.minimize(distfunction,u,du,eps,maxiter);
          } catch (IllegalStateException e) {
              throw new IllegalStateException();
          }

          p1 = c_target.getPointAt(u[0],true);
          p2 = new _Point(p_original.x + p_original.vx*u[1], p_original.y + p_original.vy*u[1], p_original.z + p_original.vz*u[1],p_original.vx,p_original.vy,p_original.vz);
          ...
          if (p1.distance(p2) < gtol) {
              ...
              p = new _Point[3];
              p[0] = new _Point(0.5f*(p1.x+p2.x),0.5f*(p1.y+p2.y),0.5f*(p1.z+p2.z));
              p[1] = p1;
              p[2] = p2;
          }
      ...
      distfunction2 = new DistancePointToCurveFunction(p_original,c_target);

      try {
          u[0] = (float)GoldenSection.minimize(0.0,1.0,distfunction2,eps);
      } catch (IllegalStateException e) {
          throw new IllegalStateException();
      }
  ...
  /**
   * Creates one or more points which is the normal projection of the p_original point onto the s_target surface
   *
   * @param p_original	Point to be projected
   * @param s_target		Surface the point is to be projected onto
   * @return				The projected point or points
   */
  public static _Point[] projectPointOntoSurface(_Point p_original, _NurbSurface s_target, float gtol) {
      ...
      if (p_original.vx != 0.0f || p_original.vy != 0.0f || p_original.vz != 0.0f) {
          ...
          // Project point along the point vector
          distfunction = new DirectedDistancePointToSurfaceFunction(p_original,s_target);
      ...
      } else
      {
          ...
          distfunction2 = new DistancePointToSurfaceFunction(p_original,s_target);

          try {
              ConstrainedDownhillSimplex.minimize(distfunction2,u,du,eps,maxiter);
          } catch (IllegalStateException e) {
              throw new IllegalStateException();
          }
      }
  ...
  /**
   * Creates one or more curves which is the normal projection of the c_original curve onto the s_target surface
   *
   * @param c_original	Curve to be projected
   * @param s_target		Surface the curve is to be projected onto
   * @return				The projected curve or curves
   */
  public static _NurbCurve[] projectCurveOntoSurface(_NurbCurve c_original, _NurbSurface s_target) {
  ...
  /**
   * Creates two curves which are the result of breaking the c_target curve at the p_breaker point
   *
   * @param c_target		Curve to be broken
   * @param p_breaker		Point (residing on curve) where the curve is to be broken
   * @return				Resulting curves after the curve has been broken
   */

  public static _NurbCurve[] breakCurve(_NurbCurve c_target, _Point p_breaker, float gtol) {
      _NurbCurve[] new_curve;
      float eps = gtol * gtol;
      float u, sz, d;
      float[] UQ, UP, UR;
      ...
      DistancePointToCurveFunction distfunction = new DistancePointToCurveFunction(p_breaker,c_target);
   ...
   * @param p           Curve degree
   * @param UP          Knot vector for curve
   * @param Pw          Control point vector for curve
   ...
   * @param UQ          Knot vector for new curve
   * @param Qw          Control point vector for new curve
   * @return uq         splitpoint index
   */
  
  public static int curveKnotIns(int p, float[] UP, _CtrlPoint[] Pw, float u, int k, int s, int r, float[] UQ, _CtrlPoint[] Qw) {
      int np = UP.length -1;
      int mp = np+p+1;
      int nq = np+r;
      _CtrlPoint[] Rw;
  ...
  /**
   * Creates a curve which is the result of joining the connected c_original curves
   *
   * @param c_original	Curves to be joined
  ...
  /**
   * Creates one or more points at the intersection points between two c_original curves
   *
   * @param c1  Curve to be intersected
   * @param c2 Second curve to be intersected
   * @return    Resulting point or points at intersection. Alternatively the point at minimum distance
   */
  public static _Point[] intersectCurves(_NurbCurve c1, _NurbCurve c2, float gtol) throws IllegalStateException {
      float[]  u = {0.5f,0.5f};
      float[] du = {0.1f,0.1f};
      _Point p1,p2;
      _Point[] p;
   ...
   *
   * @param s1, s2          Surfaces to be intersected (always 2)
   * @return                Resulting curves after intersection
   */
  public static _NurbCurve[] intersectSurfaces(_NurbSurface s1, _NurbSurface s2) throws IllegalStateException {
  ...
  /**
   * Creates one or more points at the intersection of c_original curve to the s_original surface
   * @param c_original  Curve to be intersected with surface
   * @param s_original  Surface to be intersected with curve
   * @return                Resulting point or points at intersection
   */

  public static _Point[] intersectCurveWithSurface(_NurbCurve c1, _NurbSurface s1, float gtol) throws IllegalStateException {
  ...
  /**
   * Creates one or more points on the c_target curve, placed at equal interval
   * Selecting one point will put it in the middle of the curve.
   * Selecting zero points will give the endpoints of the curve.
   *
   * @param c_target        Curve on which the points are to be determined
   * @param nr_of_points    Amount of points to be determined
   * @return                Resulting points
   */
  public static _Point[] createSpacedPoints(_NurbCurve c_target, int nr_of_points) {
  ...
  /**
   * Calulates the intersection point of two lines represented by points and direction vectors
   *
   * @param v_p1    Point 1
   * @param v_d1    Direction vector from this point
   * @param v_p2    Point 2
   * @param v_d2    Direction vector from this point
   * @return        The intersection point
   */
  public static Vector3D intersection(Vector3D p1, Vector3D v1, Vector3D p2, Vector3D v2) {
      float b,a;
      Vector3D coords = new Vector3D();
      Vector3D check = new Vector3D();

      // Solve in x/y plane
      b= (p2.y*v1.x - p1.y*v1.x - p2.x*v1.y + p1.x*v1.y) / (v2.x*v1.y - v2.y*v1.x);
      //a= (p2.x+b*v2.x-p1.x)/v1.x;

      // If failed, try y/x plane
      if (java.lang.Float.isNaN(b)) {
        b= (p2.z*v1.y - p1.z*v1.y - p2.y*v1.z + p1.y*v1.z) / (v2.y*v1.z - v2.z*v1.y);
        //a= (p2.y+b*v2.y-p1.y)/v1.y;
      }

      // If failed, try z/x plane
      if (java.lang.Float.isNaN(b)) {
        b= (p2.x*v1.z - p1.x*v1.z - p2.z*v1.x + p1.z*v1.x) / (v2.z*v1.x - v2.x*v1.z);
...
/*      check.copy(v1);
      check.scale(a);
      check.add(p1,coords);
      if (coords.z != check.z) return null;
 */
   ...
   *
   * @param vec1            First vector to be merged
   * @param vec2            Second vector to be merged
   ...
   * @return                The values not included in vec2 but present in vec1. Usually, vec2 is elevated by this value.
   */
  ...
  public static void refineKnotVectCurve(int p, float[] U, _CtrlPoint[] Pw, float[] X, float[] Ubar, _CtrlPoint[] Qw ) {
      int i,j,k,l,ind;
      int n = Pw.length - 1;
      int r = X.length - 1;
      float alfa;
      ...
      for(j=0; j<=a-p; j++) Qw[j] = new _CtrlPoint(Pw[j]);
      for(j=b-1; j<=n; j++) Qw[j+r+1] = new _CtrlPoint(Pw[j]);
      ...
      for (j=r; j>=0; j--) {
          ...
          for (l=1; l<=p; l++) {
              ...
              if (Math.abs(alfa) == 0)
                  ...
                  Qw[ind-1].x = alfa * Qw[ind-1].x + (1.0f - alfa) * Qw[ind].x;
                  Qw[ind-1].y = alfa * Qw[ind-1].y + (1.0f - alfa) * Qw[ind].y;
                  Qw[ind-1].z = alfa * Qw[ind-1].z + (1.0f - alfa) * Qw[ind].z;
                  Qw[ind-1].w = alfa * Qw[ind-1].w + (1.0f - alfa) * Qw[ind].w;
              }
          }
          Ubar[k] = X[j];
          k = k - 1;
...
public static void refineKnotVectSurface(int p, float[] U, _CtrlPoint[][] Pw, float[] X, float[] Ubar, _CtrlPoint[][] Qw ) {

    // Assume a U knot refinement.
    // Apply knot vector refinement to all v columns

    _CtrlPoint[] tmp = new _CtrlPoint[Pw.length];
    _CtrlPoint[] qtmp = new _CtrlPoint[Pw.length + X.length]; //dummy initialization
  ...
  public static int[] degreeElevateCurve(int p, float[] U, _CtrlPoint[] Pw, int t, float[] Uh, _CtrlPoint[] Qw) {
      ...
      while (b < m) {
          ...
          if (r > 0) {
              ...
              for (j=1; j<=r; j++) {
                  ...
                  s = mul+j;
                  for (k=p; k>=s; k--) {
                      bpts[k].x = alfs[k-s] * bpts[k].x + (1.0f - alfs[k-s]) * bpts[k-1].x;
                      bpts[k].y = alfs[k-s] * bpts[k].y + (1.0f - alfs[k-s]) * bpts[k-1].y;
                      bpts[k].z = alfs[k-s] * bpts[k].z + (1.0f - alfs[k-s]) * bpts[k-1].z;
                      bpts[k].w = alfs[k-s] * bpts[k].w + (1.0f - alfs[k-s]) * bpts[k-1].w;
                  }
      ...
      if (oldr > 1) {
          ...
          for (tr=1; tr<oldr; tr++) {
              ...
              while (j-i > tr) {
                  ...
                  if (i < cind) {
                      alf = (ub-Uh[i])/(ua-Uh[i]);
                      Qw[i].x = alf*Qw[i].x + (1.0f-alf)*Qw[i-1].x;
                      Qw[i].y = alf*Qw[i].y + (1.0f-alf)*Qw[i-1].y;
                      Qw[i].z = alf*Qw[i].z + (1.0f-alf)*Qw[i-1].z;
                      Qw[i].w = alf*Qw[i].w + (1.0f-alf)*Qw[i-1].w;
                  }
                  if (j >= lbz) {
                      if (j-tr <= kind-ph+oldr) {
                          gam = (ub-Uh[j-tr])/den;
                          ebpts[kj].x = gam*ebpts[kj].x + (1.0f-gam)*ebpts[kj+1].x;
                          ebpts[kj].y = gam*ebpts[kj].y + (1.0f-gam)*ebpts[kj+1].y;
                          ebpts[kj].z = gam*ebpts[kj].z + (1.0f-gam)*ebpts[kj+1].z;
                          ebpts[kj].w = gam*ebpts[kj].w + (1.0f-gam)*ebpts[kj+1].w;
                      }
                      else {
                          ebpts[kj].x = bet*ebpts[kj].x + (1.0f-bet)*ebpts[kj+1].x;
                          ebpts[kj].y = bet*ebpts[kj].y + (1.0f-bet)*ebpts[kj+1].y;
                          ebpts[kj].z = bet*ebpts[kj].z + (1.0f-bet)*ebpts[kj+1].z;
                          ebpts[kj].w = bet*ebpts[kj].w + (1.0f-bet)*ebpts[kj+1].w;
                      }
                  }
                  i = i+1;
                  j = j-1;
