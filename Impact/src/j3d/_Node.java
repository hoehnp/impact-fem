...
public class _Node extends _Object implements Serializable{
  ...
  public _Node(float xx, float yy, float zz, Color cl, Constraints con, Loads ld, boolean add) {
    x=xx;
    y=yy;
    z=zz;
    color=cl;
  ...
  public String writeObject() {
	  String st = new String();
		st = Id +"\tX = "+ x +" \tY = "+ y +" \tZ = "+ z;
		if(constraint!=null) st+=" \tConstraint = "+constraint.name;
		if(load!=null) st+=" \tLoad = "+load.name;
		if(M!=null) st+=" \tM = "+M;
		if(Ixx!=null) st+=" \tIxx = "+Ixx;
		if(Iyy!=null) st+=" \tIyy = "+Iyy;
		if(Izz!=null) st+=" \tIzz = "+Izz;
		if(Ixy!=null) st+=" \tIxy = "+Ixy;
		if(Iyz!=null) st+=" \tIyz = "+Iyz;
		if(Ixz!=null) st+=" \tIxz = "+Ixz;
  ...
  public Object[] get_Array(Canvas3D j3d){
    int k = 0;
    arr = new Object[1+(j3d.getSHOW_ID_NODE()?1:0)+((j3d.getSHOW_ID_CONSTRAINTS() && constraint!=null)?1:0)+((j3d.getSHOW_ID_LOADS() && load!=null)?1:0)];
    arr[k++]=new shpNode(x,y,z,selected ? j3d.SELECTCOLOR : color);
    if(j3d.getSHOW_ID_NODE())arr[k++]=new shpOrientedText(get_Id(),x,y,z,Color.black);
    if(j3d.getSHOW_ID_CONSTRAINTS() && constraint!=null)arr[k++]=new shpOrientedText(constraint.name,x,y,z,constraint.color);
    if(j3d.getSHOW_ID_LOADS() && load!=null)arr[k++]=new shpOrientedText(load.name,x,y,z,load.color);
...
;
     ...
     p5.add(cb_constraints, BorderLayout.CENTER);
         ...
         for(Enumeration en = pp.ConstDB.keys(); en.hasMoreElements();)  cb_constraints.addItem(en.nextElement());
         if(constraint!=null)cb_constraints.setSelectedItem(constraint.name);
     ...
     cb_loads.addItem("");
     for(Enumeration en = pp.LoadDB.keys(); en.hasMoreElements();)  cb_loads.addItem(en.nextElement());
     if(load!=null)cb_loads.setSelectedItem(load.name);
  ...
  void jButton1_actionPerformed(ActionEvent e) {
      ...
      try{
          ...
          x=xx1; y=yy1; z=zz1;
          constraint=(Constraints)PreP.ConstDB.get(cb_constraints.getSelectedItem()+"");
          load=(Loads)PreP.LoadDB.get(cb_loads.getSelectedItem()+"");
          if(tmass.getText().trim().length()!=0)M=new Float(tmass.getText());else M=null;
          if(txi.getText().trim().length()!=0)Ixx=new Float(txi.getText());else Ixx=null;
          if(tyi.getText().trim().length()!=0)Iyy=new Float(tyi.getText());else Iyy=null;
          if(tzi.getText().trim().length()!=0)Izz=new Float(tzi.getText());else Izz=null;
          if(txyi.getText().trim().length()!=0)Ixy=new Float(txyi.getText());else Ixy=null;
          if(tyzi.getText().trim().length()!=0)Iyz=new Float(tyzi.getText());else Iyz=null;
          if(txzi.getText().trim().length()!=0)Ixz=new Float(txzi.getText());else Ixz=null;
