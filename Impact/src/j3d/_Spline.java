...
public class _Spline extends _NurbCurve implements Serializable{
  ...
  public String writeObject() {
   ...
   public void readObject(String st, Hashtable geo) {
       ...
       if (r_order.search(st)) order = Integer.parseInt(r_order.stringMatched(1));
       if (r_fit.search(st)) fit_curve_to_points = (r_fit.stringMatched(1).equals("TRUE") ? true : false);
       super.readObject(st);
    }
  ...
  void jButton2_actionPerformed(ActionEvent e) {
    ...
    public _Object duplicate(Canvas3D out, boolean add) {
      ...
      for (int i=0; i<points.size(); i++) {
          o.points.set(i,(_Point)((_Point)points.elementAt(i)).duplicate(out,add));
      }
