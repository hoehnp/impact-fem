...
public class Graph extends JPanel {
  ...
  ImageIcon img_subtract = new ImageIcon(Graph.class.getResource("subtract.png"));
  ImageIcon img_multiply = new ImageIcon(Graph.class.getResource("multiply.png"));
  ImageIcon img_multiply_with_factor = new ImageIcon(Graph.class.getResource("multiply_k.png"));
  ...
  ImageIcon img_saveall = new ImageIcon(Graph.class.getResource("saveall.gif"));
  ...
  private TableModel tableModel = new AbstractTableModel() {
    public int getColumnCount() { return table_dat[0].length; }
    public int getRowCount() { return table_dat.length; }
    public Object getValueAt(int row, int col){ return table_dat[row][col]+"";}
    public String getColumnName(int col) {if(((int)(col/2))*2==col)return "X"; else return "Y";}
    public boolean isCellEditable(int row, int col) {return false;}
  };
  private TableModel tableModelInfo = new AbstractTableModel() {
    public int getColumnCount() { return 3; }
    public int getRowCount() { return table_dat_info.length; }
    public Object getValueAt(int row, int col){ if(col==1)return ""; else return table_dat_info[row][col];}
    public String getColumnName(int col) {return table_hdr_info[col];}
    public boolean isCellEditable(int row, int col) {if(col==0)return false; else return true;}
    public void setValueAt(Object aValue, int row, int col) { table_dat_info[row][col]=aValue; min_max(); p_chart.repaint();}
  ...
  JPanel p_chart = new JPanel(){
    public void paint(Graphics g){
      super.paint(g);
      offscreen = (BufferedImage)createImage(p_chart.getWidth()-100, p_chart.getHeight()-50);
      ...
      tf_maxx.setLocation(p_chart.getWidth()-80,p_chart.getHeight()-30);
      ...
      g2.drawLine(100,p_chart.getHeight()-50,p_chart.getWidth(),p_chart.getHeight()-50);
      offgraphics.setColor(Color.white);
      offgraphics.fillRect(0,0,offscreen.getWidth(),offscreen.getHeight());
      offgraphics.setColor(Color.black);
      float stepx = (p_chart.getWidth()-100)/10f;
      int j = 0;
      for(float i=100; i<=p_chart.getWidth(); i+=stepx){
        offgraphics.draw(new Line2D.Float(i-100,0,i-100,p_chart.getHeight()-50));
        String st = ""+(minx+(maxx-minx)/10f*j);
        String st1 = "";
        if(8<st.length() && st.indexOf("E")!=-1)st1 = st.substring(st.indexOf("E"),st.length());
        int index = st.indexOf(".");
        if(index!=-1 && index+8<st.length())st=st.substring(0,index+8);
        if(st1.length()!=0) st=st.substring(0,st.length()-st1.length())+st1;
      ...
      for(float i=0; i<=p_chart.getHeight()-50; i+=stepy){
        offgraphics.draw(new Line2D.Double(0,i,p_chart.getWidth(),i));
        ...
        for(j=0; j<table_dat[0].length; j+=2){
          if(((Boolean)table_dat_info[j/2][2]).booleanValue()){
            ...
            for(int i=1; i<table_dat.length; i++){
              try{
                offgraphics.drawLine((int)(Float.parseFloat(table_dat[i-1][j])*scalex+dx),
                  (int)(dy-Float.parseFloat(table_dat[i-1][j+1])*scaley),
                  (int)(Float.parseFloat(table_dat[i][j])*scalex+dx),
                  (int)(dy-Float.parseFloat(table_dat[i][j+1])*scaley)
                );
              } catch(Exception e) {}
            }
          }
  ...
  public void openXY(String file){
    try{
      ...
      while((dat=in.readLine())!=null){
        dat=dat.trim();
        if(dat.startsWith("#") || dat.length()==0) st_info+=dat.replace('\t',' ')+"\n";
        else{
      ...
      in.close();
      String[][] tmp_dat;
      if(table_dat.length==0) tmp_dat = new String[v.size()/2][2];
      else tmp_dat = new String[Math.max(table_dat.length,v.size()/2)][table_dat[0].length+2];
      ...
      for(int i=0; i<tmp_dat.length; i++){
        for(int j=0; j<2; j++){
          try{
            ...
            if(table_dat.length==0)  tmp_dat[i][j]=""+obj;
            else tmp_dat[i][table_dat[0].length+j]=""+obj;
          }catch(Exception e1) {}
        }
      }
      ...
      if(table_dat_info.length==0){
        ...
        tmp_info[table_dat_info.length][1]=this.getColorLine(tmp_info.length);
  ...
  public void addXY(String[][] arr, String st_info){
    try{
      String[][] tmp_dat;
      if(table_dat.length==0) tmp_dat = new String[arr.length][2];
      else tmp_dat = new String[Math.max(table_dat.length,arr.length)][table_dat[0].length+2];
      for(int i=0; i<tmp_dat.length; i++)
        for(int j=0; j<tmp_dat[0].length; j++)
          tmp_dat[i][j]="";
      for(int i=0; i<tmp_dat.length; i++){
  ...
  public void upd(){
    ...
    table_info = new JTable(tableModelInfo){
      ...
      public TableCellEditor getCellEditor(int row, int column) {
        if(column == 1) {
          Color cl = JColorChooser.showDialog(this,"Pick a Color",(Color)table_dat_info[row][1]);
          if(cl!=null)table_dat_info[row][1] = cl;
    ...
    table_info.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
      public void valueChanged(ListSelectionEvent e) {
        ...
        if((i=table_info.getSelectedRow())!=-1){
          ta_info.setText(""+table_dat_info[table_info.getSelectedRow()][0]);
  ...
  public void min_max(){
    ...
    for(int i=0; i<table_dat.length; i++){
      for(int j=0; j<table_dat[0].length; j+=2){
        if(((Boolean)table_dat_info[j/2][2]).booleanValue())
          try{
            minx=Math.min(minx,Float.parseFloat(table_dat[i][j]));
            maxx=Math.max(maxx,Float.parseFloat(table_dat[i][j]));
            miny=Math.min(miny,Float.parseFloat(table_dat[i][j+1]));
            maxy=Math.max(maxy,Float.parseFloat(table_dat[i][j+1]));
          } catch(Exception e) {}
      }
    }
  }
  ...
  void tf_maxy_keyPressed(KeyEvent e) {
    ...
    }else if(e.getKeyCode()==e.VK_ESCAPE)  tf_maxy.setText(maxy+"");
  }
  ...
  void tf_miny_keyPressed(KeyEvent e) {
    ...
    }else if(e.getKeyCode()==e.VK_ESCAPE)  tf_miny.setText(miny+"");
  }
  ...
  void tf_minx_keyPressed(KeyEvent e) {
    ...
    }else if(e.getKeyCode()==e.VK_ESCAPE)  tf_minx.setText(minx+"");
  }
  ...
  void tf_maxx_keyPressed(KeyEvent e) {
    ...
    }else if(e.getKeyCode()==e.VK_ESCAPE)  tf_maxx.setText(maxx+"");
  }
  ...
  public void save_Image(String file) {
    ...
    try {
      ...
      BufferedImage image = (BufferedImage)createImage(p_chart.getWidth(), p_chart.getHeight());
  ...
  void del_action() {
    ...
    for(int i=0; i<table_dat_info.length; i++){
      if(index!=i){
        for(int j=0; j<3; j++)tmp_info[z][j]=table_dat_info[i][j];
        z++;
      }
    }
    z=0;
      ...
      try {
          ...
          if(choise == JFileChooser.APPROVE_OPTION && fd.getSelectedFile()!=null){
              ...
              if (fd.getFileFilter().equals(f1)) {
                  ...
                  f.mkdir();
                  String st = "<html><b><font size=+2><center>Result - "+new Date()+"</center></font></b><br><img src=index.png><br><hr>";
                  for(int i=0; i<table_dat_info.length; i++){
                      if(((Boolean)table_info.getValueAt(i,2)).booleanValue()==true){
                          st+="<pre>"+table_dat_info[i][0]+"</pre><img src="+i+".jpg><a href="+i+".trk>Show data</a><hr>";
                          RandomAccessFile o = new RandomAccessFile(file+File.separator+i+".trk", "rw");
                          o.writeBytes(table_dat_info[i][0]+"\n");
                          for(int k=0; k<table_dat.length; k++)o.writeBytes(table_dat[k][i*2]+"   "+table_dat[k][i*2+1]+"\n");
                          o.close();
                          this.save_ImageColor(file+File.separator+i+"",((Color)table_dat_info[i][1]));
                      }
                      repaint();
                      this.save_Image(file+File.separator+"index.png");
                  }
                  RandomAccessFile out = new RandomAccessFile(file+File.separator+"index.html", "rw");
      ...
      try {
          ...
          if(choise == JFileChooser.APPROVE_OPTION && fd.getSelectedFile()!=null){
              ...
              if (fd.getFileFilter().equals(f1)) {
                  ...
                  f.mkdir();
                  String st = "<html><b><font size=+2><center>Result - "+new Date()+"</center></font></b><br><br><hr>";
                  for(int i=0; i<table_dat_info.length; i++){
                      st+="<pre>"+table_dat_info[i][0]+"</pre><br><a href="+i+".trk>Show data</a><br><img src="+i+".png><hr><br><br>";
                      for(int j=0; j<table_dat_info.length; j++){
                          if(i==j){
                              table_info.setValueAt(new Boolean(true),j,2);
                              RandomAccessFile o = new RandomAccessFile(file+File.separator+i+".trk", "rw");
                              o.writeBytes(table_dat_info[i][0]+"\n");
                              for(int k=0; k<table_dat.length; k++)o.writeBytes(table_dat[k][j*2]+"   "+table_dat[k][j*2+1]+"\n");
                              o.close();
                          }else table_info.setValueAt(new Boolean(false),j,2);
                      }
                      repaint();
  ...
  void b_union_actionPerformed(ActionEvent e) {
    ...
    for(int i=0; i<table_dat.length; i++){
      ...
      for(int j=0; j<index.length; j++){
        if(arr[i][0].length()!=0) if(arr[i][1]==null || arr[i][1].length()==0)arr[i][1]=""+Float.parseFloat(table_dat[i][index[j]*2+1]);
        else if(table_dat[i][index[j]*2+1].length()>0)arr[i][1]=""+(Float.parseFloat(arr[i][1])+Float.parseFloat(table_dat[i][index[j]*2+1]));
      }
    }
  ...
  void b_subtract_actionPerformed(ActionEvent e) {
      int index[] = table_info.getSelectedRows();
      if (inverted) index = this.invert(index);
      String[][] arr = new String[table_dat.length][2];
  ...
  void b_multiply_with_K_actionPerformed(ActionEvent e) {
      ...
      for(int i=0; i<table_dat.length; i++){
        arr[i][0]=table_dat[i][index[0]*2];
        for(int j=0; j<index.length; j++){
          if(arr[i][0].length()!=0) if(arr[i][1]==null || arr[i][1].length()==0)arr[i][1]=""+(Float.parseFloat(table_dat[i][index[j]*2+1])*factor);
          else if(table_dat[i][index[j]*2+1].length()>0)arr[i][1]=""+(Float.parseFloat(arr[i][1])*factor);
        }
      }
      this.addXY(arr,arr_info);
    }
  ...
  void tf_factor_keyPressed(KeyEvent e) {
      ...
      }else if(e.getKeyCode()==e.VK_ESCAPE)  tf_factor.setText(factor+"");
    }
  ...
  void b_value_vs_value_actionPerformed(ActionEvent e) {
      ...
      for(int i=0; i<table_dat.length; i++){
        ...
        for(int j=0; j<index.length; j++){
          if(arr[i][0].length()!=0) if(arr[i][1]==null || arr[i][1].length()==0)arr[i][1]=""+Float.parseFloat(table_dat[i][index[j]*2+1]);
          else if(table_dat[i][index[j]*2+1].length()>0)arr[i][1]=""+(Float.parseFloat(table_dat[i][index[j]*2+1]));
        }
      }
      this.addXY(arr,arr_info);
    }
  ...
  void b_inv_actionPerformed(ActionEvent e) {
      ...
      if (inverted) b_inv.setIcon(img_flop);
      else b_inv.setIcon(img_flip);
