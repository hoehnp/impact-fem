...
public class NodeAccelerationTracker extends Tracker implements uka.patch.Patchable, uka.transport.Transportable {
   public void createPatch(Object _copy, uka.patch.PatchOutput po) throws java.io.IOException {
      NodeAccelerationTracker copy = (NodeAccelerationTracker)_copy;
   ...
   public static final uka.transport.TransportDescriptor TRANSPORT_DESCRIPTOR = new uka.transport.TransportDescriptor(){
      ...
      public boolean deepCloneReferences(Object orig, Object copy, uka.transport.DeepClone _helper) throws CloneNotSupportedException {
         ((NodeAccelerationTracker)copy).deepCloneReferences((NodeAccelerationTracker)orig, _helper);
         return false;
      }
      public Class getType() {
         return NodeAccelerationTracker.class;
      }
   };
   ...
   public NodeAccelerationTracker(uka.transport.UnmarshalStream _stream, int _id) throws java.io.IOException, ClassNotFoundException {
      super(_stream, _id);
   }
   public void unmarshal(uka.transport.UnmarshalStream _stream) throws java.io.IOException, ClassNotFoundException {
      ...
      _stream.request(NodeAccelerationTracker._SIZE);
      ...
      _pos += uka.transport.BasicIO.SIZEOF_boolean;
      _stream.accept(NodeAccelerationTracker._SIZE);
   ...
   public void marshal(uka.transport.MarshalStream _stream) throws java.io.IOException {
      ...
      _stream.reserve(NodeAccelerationTracker._SIZE);
      byte[] _buffer = _stream.getBuffer();
      int _pos = _stream.getPosition();
   ...
   public NodeAccelerationTracker(NodeAccelerationTracker _orig, int _id, uka.transport.DeepClone _helper) throws CloneNotSupportedException, java.io.IOException {
   ...
   public void deepCloneReferences(NodeAccelerationTracker _orig, uka.transport.DeepClone _helper) throws CloneNotSupportedException {
   ...
   public NodeAccelerationTracker() {
      super();
      this.node = new Node();
   }
   ...
   private void collectOwnData() throws IllegalArgumentException {
      int i;
      if (this.print_x) {
         this.result = this.node.getX_acc();
      } else if (this.print_y) {
         this.result = this.node.getY_acc();
      } else {
         this.result = this.node.getZ_acc();
      }
   }
   ...
   public void parse_Fembic(Token[] param, int lineno, RplVector globalnodelist, RplVector globalelementlist) throws java.text.ParseException {
      ...
      while (i < param.length) {
         ...
         } else if (param[i].getw().toUpperCase().equals("DIRECTION") && param[i + 1].getw().toUpperCase().equals("=") && param[i + 2].getw().toUpperCase().equals("X")) {
            this.print_x = true;
            i += 3;
         } else if (param[i].getw().toUpperCase().equals("DIRECTION") && param[i + 1].getw().toUpperCase().equals("=") && param[i + 2].getw().toUpperCase().equals("Y")) {
            this.print_y = true;
            i += 3;
         } else if (param[i].getw().toUpperCase().equals("DIRECTION") && param[i + 1].getw().toUpperCase().equals("=") && param[i + 2].getw().toUpperCase().equals("Z")) {
            this.print_z = true;
            i += 3;
   ...
   public void checkIndata() throws IllegalArgumentException {
      if (!this.Nodes_are_set) {
         throw new IllegalArgumentException("No nodes defined for this node acceleration tracker: " + this.number);
      }
      if (!this.print_x && !this.print_y && !this.print_z) {
         throw new IllegalArgumentException("No directions defined for this node acceleration tracker: " + this.number);
      }
      if (!this.File_is_set) {
         throw new IllegalArgumentException("No Filename defined for this node acceleration tracker: " + this.number);
      }
   }
   public void print_Gid(int ctrl, double currtime) throws IOException, IllegalArgumentException {
      ...
      case run.Tracker.RESULT_HEADER: 
         try {
            ...
            out = new String("# Impact node acceleration tracker results from tracker number: " + this.number + "\n");
            out += "# The results are from node: " + this.node.getNumber() + "\n";
            out += "# \n";
            out += "# X: time \t Y: acceleration in direction: ";
            if (this.print_x) {
               out += "X";
            }
            if (this.print_y) {
            ...
            System.out.println("Error in creating the NodeAccelerationTracker file: " + this.filename);
            throw ioe;
         }
         return;
      ...
      case run.Tracker.RESULT: 
         try {
            ...
            System.out.println("Error in writing results to the NodeAccelerationTracker file: " + this.filename);
            throw ioe;
         }
         return;
      
      default: 
         throw new IllegalArgumentException("Unknown parameter for print_Gid in NodeAccelerationTracker number: " + this.number);
   ...
   public int getType() {
      return run.trackers.NodeAccelerationTracker.TYPE;
   }
}
