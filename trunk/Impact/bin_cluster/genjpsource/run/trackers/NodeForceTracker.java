...
public class NodeForceTracker extends Tracker implements uka.patch.Patchable, uka.transport.Transportable {
   public void createPatch(Object _copy, uka.patch.PatchOutput po) throws java.io.IOException {
      NodeForceTracker copy = (NodeForceTracker)_copy;
   ...
   public static final uka.transport.TransportDescriptor TRANSPORT_DESCRIPTOR = new uka.transport.TransportDescriptor(){
      ...
      public boolean deepCloneReferences(Object orig, Object copy, uka.transport.DeepClone _helper) throws CloneNotSupportedException {
         ((NodeForceTracker)copy).deepCloneReferences((NodeForceTracker)orig, _helper);
         return false;
      }
      public Class getType() {
         return NodeForceTracker.class;
      }
   };
   ...
   public NodeForceTracker(uka.transport.UnmarshalStream _stream, int _id) throws java.io.IOException, ClassNotFoundException {
      super(_stream, _id);
   }
   public void unmarshal(uka.transport.UnmarshalStream _stream) throws java.io.IOException, ClassNotFoundException {
      ...
      _stream.request(NodeForceTracker._SIZE);
      ...
      _pos += uka.transport.BasicIO.SIZEOF_boolean;
      _stream.accept(NodeForceTracker._SIZE);
   ...
   public void marshal(uka.transport.MarshalStream _stream) throws java.io.IOException {
      ...
      _stream.reserve(NodeForceTracker._SIZE);
      byte[] _buffer = _stream.getBuffer();
      int _pos = _stream.getPosition();
   ...
   public NodeForceTracker(NodeForceTracker _orig, int _id, uka.transport.DeepClone _helper) throws CloneNotSupportedException, java.io.IOException {
   ...
   public void deepCloneReferences(NodeForceTracker _orig, uka.transport.DeepClone _helper) throws CloneNotSupportedException {
   ...
   private void collectOwnData() throws IllegalArgumentException {
      ...
      for (i = 0; i < this.nodelist.size(); i++) {
         ...
         if (this.print_x && !this.positive && !this.negative) {
            this.resultlist[i] = tempnode.getX_force();
         } else if (this.print_x && this.positive) {
            this.resultlist[i] = tempnode.getX_force_component(true);
         } else if (this.print_x && this.negative) {
            this.resultlist[i] = tempnode.getX_force_component(false);
         } else if (this.print_y && !this.positive && !this.negative) {
            this.resultlist[i] = tempnode.getY_force();
         } else if (this.print_y && this.positive) {
            this.resultlist[i] = tempnode.getY_force_component(true);
         } else if (this.print_y && this.negative) {
            this.resultlist[i] = tempnode.getY_force_component(false);
         } else if (this.print_z && !this.positive && !this.negative) {
            this.resultlist[i] = tempnode.getZ_force();
         } else if (this.print_z && this.positive) {
            this.resultlist[i] = tempnode.getZ_force_component(true);
         } else if (this.print_z && this.negative) {
            this.resultlist[i] = tempnode.getZ_force_component(false);
         }
      }
   }
   ...
   public void parse_Gmsh(Token[] param, int lineno, RplVector globalnodelist, RplVector globalelementlist) throws java.text.ParseException {
      ...
      for (j = 5; j < param.length; j++) {
         this.nodelist.add(super.findNode((int)param[j].getn(), globalnodelist));
      }
      this.Nodes_are_set = true;
      this.filename = new String("dummmy.trk");
      this.File_is_set = true;
      this.print_x = true;
   }
   public void checkIndata() throws IllegalArgumentException {
      if (!this.Nodes_are_set) {
         throw new IllegalArgumentException("No nodes defined for this node force tracker: " + this.number);
      }
      if (!this.print_x && !this.print_y && !this.print_z) {
         throw new IllegalArgumentException("No directions defined for this node force tracker: " + this.number);
      }
      if (!this.File_is_set) {
         throw new IllegalArgumentException("No Filename defined for this node force tracker: " + this.number);
      }
   }
   public void print_Gid(int ctrl, double currtime) throws IOException, IllegalArgumentException {
      ...
      case run.Tracker.RESULT_HEADER: 
         try {
            ...
            out = new String("# Impact node force tracker results from tracker number: " + this.number + "\n");
            ...
            System.out.println("Error in creating the NodeForceTracker file: " + this.filename);
            throw ioe;
         }
         return;
      ...
      case run.Tracker.RESULT: 
         try {
            ...
            System.out.println("Error in writing results to the NodeForceTracker file: " + this.filename);
            throw ioe;
         }
         return;
      
      default: 
         throw new IllegalArgumentException("Unknown parameter for print_Gid in NodeForceTracker number: " + this.number);
   ...
   public int getType() {
      return run.trackers.NodeForceTracker.TYPE;
   }
}
