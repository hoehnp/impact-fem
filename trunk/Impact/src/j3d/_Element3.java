...
public void reset(boolean do_mesh) {};
  ...
  public _Element3(_Node n1, _Node n2, _Node n3, int t, Material m, float th){
    msh_type=t;
    if(t==Canvas3D.MESH_Contact_Triangle)msh_name = "Contact_Triangle";
    else msh_name = "Shell_C0_3";
    node1=n1;
    node2=n2;
    node3=n3;
    material=m;
  ...
  public String writeObject() {
    ...
    st+=" \tnodes = ["+node1.get_Id()+","+node2.get_Id()+","+node3.get_Id()+"] \tT = "+thickness;
    if(factor!=null) st+=" \tFactor = "+factor;
    if(friction!=null) st+=" \tFriction = "+friction;

    if (msh_type == Canvas3D.MESH_Contact_Triangle) return st;
    
    if(material != null) st +=" \tMaterial = " +material.name;
    if(NIP!=null && msh_type != Canvas3D.MESH_Contact_Triangle) st+=" \tNIP = "+NIP;
  ...
  public Object[] get_Array(Canvas3D j3d){
    int k=0;
    arr = new Object[1+(j3d.getSHOW_ID_ELEMENT()?1:0)+((j3d.getSHOW_ID_LOADS() && msh_type!=Canvas3D.MESH_Contact_Triangle && load!=null)?1:0)];
    if(material==null)arr[k++]=new shpTriangle(node1.x,node1.y,node1.z,node2.x,node2.y,node2.z,node3.x,node3.y,node3.z,selected ? Canvas3D.SELECTCOLOR : null, contur);
    else arr[k++]=new shpTriangle(node1.x,node1.y,node1.z,node2.x,node2.y,node2.z,node3.x,node3.y,node3.z,selected ? Canvas3D.SELECTCOLOR : material.color, selected ? Canvas3D.SELECTCOLOR : material.color);

    if(j3d.getSHOW_ID_ELEMENT()){
      arr[k++]=new shpOrientedText(get_Id(),(node1.x+node2.x+node3.x)/3,(node1.y+node2.y+node3.y)/3,(node1.z+node2.z+node3.z)/3,contur);
    }

    if(j3d.getSHOW_ID_LOADS() && msh_type!=Canvas3D.MESH_Contact_Triangle && load!=null) arr[k++]=new shpOrientedText(load.name,(node1.x+node2.x+node3.x)/3,(node1.y+node2.y+node3.y)/3,(node1.z+node2.z+node3.z)/3,load.color);
  ...
  void jButton1_actionPerformed(ActionEvent e) {
      try{
          ...
          if(node1_tmp==null || node2_tmp==null || node3_tmp==null){error("Node = null!"); return;}
          float thickness_tmp = java.lang.Float.parseFloat(tft.getText());
          java.lang.Float factor_tmp=tffact.getText().trim().length()==0?null:new java.lang.Float(tffact.getText());
          java.lang.Float friction_tmp=tffrict.getText().trim().length()==0?null:new java.lang.Float(tffrict.getText());
          Integer NIP_tmp=tfnip.getText().trim().length()==0?null:new Integer(tfnip.getText());
          Integer PIP_tmp=tfpip.getText().trim().length()==0?null:new Integer(tfpip.getText());
          node1=node1_tmp;
          node2=node2_tmp;
          node3=node3_tmp;
          msh_name=cb_type.getSelectedItem()+"";
          material = (Material)PreP.MatDB.get(cb_mat.getSelectedItem()+"");
          if(cb_type.getSelectedIndex()==0) msh_type = Canvas3D.MESH_Contact_Triangle; else msh_type = Canvas3D.MESH_Shell_C0_3;
          contact=(cb_ctype.getSelectedItem()+"").length()==0?null:cb_ctype.getSelectedItem()+"";
...
public double getMinAngle() {
	...
	v1 = new Vector3D(node2.x-node1.x, node2.y-node1.y, node2.z-node1.z);
	v2 = new Vector3D(node3.x-node1.x, node3.y-node1.y, node3.z-node1.z);
	v3 = new Vector3D(node3.x-node2.x, node3.y-node2.y, node3.z-node2.z);
...
public double getMinLength() {
    ...
    return Math.min(v3.getLength(),Math.min(v1.getLength(),v2.getLength()));
}
...
public double getMaxLength() {
    ...
    return Math.max(v3.getLength(),Math.max(v1.getLength(),v2.getLength()));
}
