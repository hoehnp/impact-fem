...
private void createGroups() {
  ...
  public synchronized void paint(Graphics g){
    ...
    if (DRAG == true) {
        ...
        offgraphics.drawRect(Math.min(x0,x),Math.min(y0,y),Math.abs(x-x0),Math.abs(y-y0));
    }
  ...
  public void removeSelectedObjects3D(){
      ...
      for(int i=0; i<obj3d.size(); i++) {
          o = (_Object)obj3d.elementAt(i);
          if(o.isSelected()){
              for (int j=0; j<obj3d.size(); j++)  // Remove from all groups
                  if (obj3d.elementAt(j) instanceof _Group)
                      ((_Group)obj3d.elementAt(j)).removeFromGroup(o);
              obj3d.removeElementAt(i);
              i--;
          } else 
  ...
  public synchronized void view_all(){
      ...
      for(int i=0; i<shp3d.size(); i++){
          try{
              ...
              if (!(shp instanceof shpPointGrid) && !(shp instanceof shpOrientedText)) { 
  ...
  public synchronized void view_grid(){
    ...
    if(GRIDMODE)for(float i=LIMITS[0]; i<=LIMITS[2]; i+=GRIDSIZE){
  ...
  void this_mouseDragged(MouseEvent e) {
      ...
      if(((mod & InputEvent.BUTTON3_MASK) == InputEvent.BUTTON3_MASK) && ((mod & InputEvent.CTRL_MASK) == InputEvent.CTRL_MASK) && ((mod & InputEvent.SHIFT_MASK) == InputEvent.SHIFT_MASK)) {  
  ...
  public void setCenterOfRotation() {
      ...
      if (obj != null) {
         ...
         center_of_rotation.sub(new Vector3D(0.0f,0.0f,0.0f),center_of_rotation);
