 ...
 * Computer Methods in Applied Mechanics and Engineering 42 (1984) 225-251
 * Dyna Theoretical Manual Radioss Manual
 *
...
public class Shell_BT_4 extends Element {
        ...
        throws IllegalArgumentException
    {
        ...
        /* Now, calculate the inertia.
           [Ixx  Iyx  Izx]
           I = [Ixy  Iyy  Izy]
               [Ixz  Iyz  Izz]
           With the exact formula for intertia, the solution tends to diverge
           for large rotation rates. A way to stabalize the rotation is to let
           Izz = Iyy = Ixx , Ixy... = 0 and concider the rectangle as a square.
    ...
    public void calculateExternalForces(double currtime) {
        ...
        // Add the pressure force in local z-direction if there is a load object defined
        if (load != null) {
            force.set(0, 0, 0);
            force.set(1, 0, 0);
            ...
            // Distribute on nodes (note this is external forces so no sign reversal)
            for (i = 0; i < 4; i++) {
        ...
        if ((Contact == EDGE) || (Contact == ADVANCED_EDGE)) {
            if (internal_contact_line_element_1 != null) {
                internal_contact_line_element_1.calculateExternalForces(
                    currtime
                );
            }

            if (internal_contact_line_element_2 != null) {
                internal_contact_line_element_2.calculateExternalForces(
                    currtime
                );
            }

            if (internal_contact_line_element_3 != null) {
                internal_contact_line_element_3.calculateExternalForces(
                    currtime
                );
            }

            if (internal_contact_line_element_4 != null) {
                internal_contact_line_element_4.calculateExternalForces(
                    currtime
                );
            }
        }
     ...
     * base vectors for the element. These vectors defines the local
     * coordinate system for the element on which all the stress, strain and
     * force calculations are based. 
     * 
     * The input are four points in space. They are used as follows:
     * 1. The first point is the base point.
     * 2. The local x-axis is defined from base point to the second point.
     * 3. The local z-axis is defined as the cross product of the diagonals 1-3, 2-4
     * 
     * 
     * 
     * 
     ...
     * 
     * 
     * 3. The local y-axis is defined from the base point to the third point 4. The cross
     * vector between the x-axis and the y axis, defines the normal to the
     * plane 5. A new adjusted x-axis is calculated. 6. Finally, the y-axis is
     * calculated as the cross product of the x-axis and z-axis. Now, all the
     * three axis are normal to each other and a correct coordinate system is
     * defined. The method will return a matrix with these vectors: x-vec
     * y-vec    z-vec     x             y         z         Creation date:
     * (26/08/01 %T) Jonas Forssell
     */
    public void calculateLocalBaseVectors(
        ...
        // local_y_axis = local_x_axis.times(local_z_axis.transpose()).times(local_z_axis);
        bvs.set(
            1, 0,
     ...
     * loads Thus, the "addition" is negative. Creation date: (2001-10-23
     * 14.03.06)
     *
     */
    public void calculateNodalForces(int integration_point, double timestep)
    {
        /* The nodal forces are computed as a function of the gauss integration of the
           stresses. This means a summary must be made for all integration points.
           This routine will be called once for every integration point so this routine
           only has to calculate the force contribution of a specific integration point and
           distribute it on the nodes. */
        int i;
        double ki;
        double ko;
        double kr;
        ...
        for (i = 0; i < 4; i++) {
            ...
            force.set(
                0, 0,
                0.5 * thickness * area * weight_factor[integration_point] * ((B.get(0,
                    i
                ) * stress[integration_point].get(0, 0)) +
                (B.get(1, i) * stress[integration_point].get(3, 0)))
            );
            force.set(
                1, 0,
                0.5 * thickness * area * weight_factor[integration_point] * ((B.get(1,
                    i
                ) * stress[integration_point].get(1, 0)) +
                (B.get(0, i) * stress[integration_point].get(3, 0)))
            );
            force.set(
                2, 0,
                0.5 * thickness * area * shear_factor * weight_factor[integration_point] * ((B.get(0,
                    i
                ) * stress[integration_point].get(5, 0)) +
                (B.get(1, i) * stress[integration_point].get(4, 0)))
            );

            /*         Now, add the hourglass forces
               These are calculated according to a viscous model where the forces are related to the
               rate of displacement (node velocity). The formulation is based on work done by
               Kosloff and Frasier.
             */
            if ((integration_point == 0) && (hourglass_control == true)) {
                if (i == 0) {
                    ki = 0.25 * material[0].getDensity() * material[0].wavespeedTwoDimensional(
                            0.0, 0.0
                        ) * thickness * Math.sqrt(
                            (membrane_hourglass_coeff * area) / 2
                        );
                    ko = 0.25 * material[0].getDensity() * material[0].wavespeedTwoDimensional(
                            0.0, 0.0
                        ) * thickness * thickness * Math.sqrt(
                            out_of_plane_hourglass_coeff / 10
                        );
                    kr = 0.02 * material[0].getDensity() * material[0].wavespeedTwoDimensional(
                            0.0, 0.0
                        ) * thickness * thickness * area * Math.sqrt(
                            rotation_hourglass_coeff / 2
                        );
                }
                ...
                hourglass_force.set(
                    0, 0,
                    ki * ((v.get(0, 0) - v.get(3, 0) + v.get(6, 0)) -
                    v.get(9, 0)) * hourglass_vector.get(i, 0)
                );
                hourglass_force.set(
                    1, 0,
                    ki * ((v.get(1, 0) - v.get(4, 0) + v.get(7, 0)) -
                    v.get(10, 0)) * hourglass_vector.get(i, 0)
                );

                // Out-of-plane mode (z directions)
                hourglass_force.set(
                    2, 0,
                    ko * ((v.get(2, 0) - v.get(5, 0) + v.get(8, 0)) -
                    v.get(11, 0)) * hourglass_vector.get(i, 0)
                );

                // .. and transform to global coordinate system
                hourglass_force = transposed_coordinate_system.times(
                        hourglass_force
                    );
            ...
            // For those who are studying this in detail and is wondering why the formula is 0.5*thickness and not 0.25*thickness*thickness, remember that z[integration_point] already includes 0.5*thickness
            moment.set(
                0, 0,
                0.5 * thickness * area * weight_factor[integration_point] * (((B.get(1,
                    i
                ) * (-z[integration_point] * stress[integration_point].get(
                    1, 0
                ))) +
                (B.get(0, i) * (-z[integration_point] * stress[integration_point].get(
                    3, 0
                )))) -
                (0.25 * shear_factor * stress[integration_point].get(4, 0)))
            );
            moment.set(
                1, 0,
                0.5 * thickness * area * weight_factor[integration_point] * ((-B.get(0,
                ...
                (B.get(1, i) * (-z[integration_point] * stress[integration_point].get(
                    3, 0
                ))) +
                (0.25 * shear_factor * stress[integration_point].get(5, 0)))
            );
            moment.set(2, 0, 0);

            // Hourglass
            if ((integration_point == 0) && (hourglass_control == true)) {
                hourglass_moment.set(
                    0, 0,
                    kr * ((a.get(0, 0) - a.get(3, 0) + a.get(6, 0)) -
                    a.get(9, 0)) * hourglass_vector.get(i, 0)
                );
                hourglass_moment.set(
                    1, 0,
                    kr * ((a.get(1, 0) - a.get(4, 0) + a.get(7, 0)) -
                    a.get(10, 0)) * hourglass_vector.get(i, 0)
                );
                hourglass_moment.set(
                    2, 0,
                    kr * ((a.get(2, 0) - a.get(5, 0) + a.get(8, 0)) -
                    a.get(11, 0)) * hourglass_vector.get(i, 0)
                );

                // .. and transform to global coordinate system
                hourglass_moment = transposed_coordinate_system.times(
                        hourglass_moment
                    );
            ...
            /* Now, add the force and moment loads to the node. Remember that internal forces are
               negative and external are positive. */
    ...
    /**
     * v is an array of node velocities expressed in the local coordinate
     * system v = [u1 v1 w1 u2 v2 w2 u3 v3 w3 u4 v4 w4] p is an array of node
    ...
    public void calculateStrain(double tstep, int integration_point) {
        // Do this block only once for each timestep. Not for each integration point
        if (integration_point == 0) {
            this.updateLocalCoordinateSystem();
            this.calculateLocalVariables();
            ...
            /* Now that we have the input in local coordinate system, it
               is time to calculate the strain rates.
               Start by creating the B matrix (the derivate of the shape functions)
               which will be used frequently in the rest of the calculations. */
            ...
            /* Now, the strain rates can be determined. They are calculated in
               the middle of the shell and then transformed out through the thickness.
            ...
            //
            kappax = -((B.get(0, 0) * (a.get(1, 0) - a.get(7, 0))) +
                (B.get(0, 1) * (a.get(4, 0) - a.get(10, 0))));
        ...
        strainrate.set(
            4, 0,
            ((B.get(1, 0) * (v.get(2, 0) - v.get(8, 0))) +
            (B.get(1, 1) * (v.get(5, 0) - v.get(11, 0)))) -
            (0.25 * (a.get(0, 0) + a.get(3, 0) + a.get(6, 0) + a.get(9, 0)))
        );
        ...
        strainrate.set(
            ...
            (0.25 * (a.get(1, 0) + a.get(4, 0) + a.get(7, 0) + a.get(10, 0)))
        );

        // Knowing the strainrate, the strain increase for this integration point is easily computed.
    ...
    public void calculateStress(int integration_point, double timestep) {
        /* This calculates the stresses using the strain and strain increase.
           The result is put back into the stress matrix. The element contains one local
           material object for each integration point since some materials has a memory which must
           be maintaned. */
        ...
        // Update the thickness for thickness reduction using the mid-most integration point.
        if (
            Thinning_is_enabled &&
            (integration_point == (number_of_integration_points / 2))
        ) {
            thickness = (1 +
                strain[number_of_integration_points / 2].get(2, 0)) * original_thickness;
        }
    }
    ...
    public double checkTimestep(double current_timestep) {
        ...
        critical_length = Math.sqrt(
                ((node[1].getX_pos() - node[0].getX_pos()) * (node[1].getX_pos() -
                node[0].getX_pos())) +
                ((node[1].getY_pos() - node[0].getY_pos()) * (node[1].getY_pos() -
                node[0].getY_pos())) +
                ((node[1].getZ_pos() - node[0].getZ_pos()) * (node[1].getZ_pos() -
                node[0].getZ_pos()))
            );
        critical_length = Math.max(
                critical_length,
                Math.sqrt(
                    ((node[2].getX_pos() - node[1].getX_pos()) * (node[2].getX_pos() -
                    node[1].getX_pos())) +
                    ((node[2].getY_pos() - node[1].getY_pos()) * (node[2].getY_pos() -
                    node[1].getY_pos())) +
                    ((node[2].getZ_pos() - node[1].getZ_pos()) * (node[2].getZ_pos() -
                    node[1].getZ_pos()))
                )
            );
        critical_length = Math.max(
                critical_length,
                Math.sqrt(
                    ((node[3].getX_pos() - node[2].getX_pos()) * (node[3].getX_pos() -
                    node[2].getX_pos())) +
                    ((node[3].getY_pos() - node[2].getY_pos()) * (node[3].getY_pos() -
                    node[2].getY_pos())) +
                    ((node[3].getZ_pos() - node[2].getZ_pos()) * (node[3].getZ_pos() -
                    node[2].getZ_pos()))
                )
            );
        critical_length = Math.max(
                critical_length,
                Math.sqrt(
                    ((node[0].getX_pos() - node[3].getX_pos()) * (node[0].getX_pos() -
                    node[3].getX_pos())) +
                    ((node[0].getY_pos() - node[3].getY_pos()) * (node[0].getY_pos() -
                    node[3].getY_pos())) +
                    ((node[0].getZ_pos() - node[3].getZ_pos()) * (node[0].getZ_pos() -
                    node[3].getZ_pos()))
                )
            );
        ...
        timestep = critical_length / material[0].wavespeedTwoDimensional(
                0.0, 0.0
            );

        // Now, return the smallest of the suggested and the calculated timestep. 
        // The smallest element will decide the timestep for the whole simulation.
        ...
        throws java.text.ParseException
    {
        ...
        while (i < param.length) {
            ...
            if (
                param[i].getw().toUpperCase().equals("MATERIAL") &&
                param[i + 1].getw().toUpperCase().equals("=")
            ) {
                // Assume now that the material name is delivered in param3
                try {
                    // We want the handle to the material object.
                    material[0] = super.findMaterial(
                            param[i + 2].getw().toUpperCase(), materiallist
                        );
                    i += 3;
            ...
            if (
                param[i].getw().toUpperCase().equals("NIP") &&
                param[i + 1].getw().toUpperCase().equals("=")
            ) {
                // The value of the cross section area is in param3. Set this in the element
            ...
            if (
                param[i].getw().toUpperCase().equals("PIP") &&
                param[i + 1].getw().toUpperCase().equals("=")
            ) {
                // The value of the cross section area is in param3. Set this in the element
            ...
            if (
                param[i].getw().toUpperCase().equals("T") &&
                param[i + 1].getw().toUpperCase().equals("=")
            ) {
                // The value of the element thickness is in param3. Set this in the element
            ...
            if (
                param[i].getw().toUpperCase().equals("SHEAR_FACTOR") &&
                param[i + 1].getw().toUpperCase().equals("=")
            ) {
            ...
            if (
                param[i].getw().toUpperCase().equals("HOURGLASS") &&
                param[i + 1].getw().toUpperCase().equals("=")
            ) {
                // Default is horglass control is ON. Anything other than OFF means default.
            ...
            if (
                param[i].getw().toUpperCase().equals("MHC") &&
                param[i + 1].getw().toUpperCase().equals("=")
            ) {
            ...
            if (
                param[i].getw().toUpperCase().equals("OOPHC") &&
                param[i + 1].getw().toUpperCase().equals("=")
            ) {
            ...
            if (
                param[i].getw().toUpperCase().equals("RHC") &&
                param[i + 1].getw().toUpperCase().equals("=")
            ) {
            ...
            if (
                param[i].getw().toUpperCase().equals("CONTACT") &&
                param[i + 1].getw().toUpperCase().equals("=")
            ) {
                // The value of the element thickness is in param3. Set this in the element
                if (param[i + 2].getw().toUpperCase().equals("OFF")) {
                    ...
                    param[i + 2].getw().toUpperCase().equals("ADVANCED_EDGE")
                ) {
            ...
            if (
                param[i].getw().toUpperCase().equals("THINNING") &&
                param[i + 1].getw().toUpperCase().equals("=")
            ) {
                ...
                if (param[i + 2].getw().toUpperCase().equals("OFF")) {
                    ...
                    throw new ParseException(
                        "Unrecognized thinning parameter", lineno
                    );
                }
        ...
        if ((Contact == BASIC) || (Contact == EDGE)) {
            ...
            internal_contact_element_1.parse_Fembic(
                contact_input, lineno, nodelist, materiallist, loadlist, nodetable
            );

            // Make the input for the contact element 2
            contact_input[2] = new Token(
                    new String(
                        "[" + node[2].getNumber() + "," + node[3].getNumber() +
                        "," + node[0].getNumber() + "]"
                    )
                );

            // Now, set the input for the contact element also
            internal_contact_element_2.parse_Fembic(
                contact_input, lineno, nodelist, materiallist, loadlist, nodetable
            );
        }
        ...
        if ((Contact == ADVANCED) || (Contact == ADVANCED_EDGE)) {
            ...
            // Note that it is an internal node which makes it special compared to a
            // standard node.
            // Use a negative number which is same as element number to make it unique

            middle_node = new InternalNode();
            middle_node.setNumber(-number);
            middle_node.registerMasterElement(this);
            
            // Add the internal node to the nodelist so it becomes part of the main loop
            ...
            internal_contact_element_3.parse_Fembic(
                contact_input, lineno, nodelist, materiallist, loadlist, nodetable
            );
            ...
            internal_contact_element_4.parse_Fembic(
                contact_input, lineno, nodelist, materiallist, loadlist, nodetable
            );

        }

        /* Make local instances of the edges contact elements to handle the contacts.
         * Note that some of the contact_inputs are assumed set in the previous block */
        if ((Contact == EDGE) || (Contact == ADVANCED_EDGE)) {
            if (! node[0].hasContact_LineElementConnectedTo(node[1])) {
                ...
                // Set the input
                internal_contact_line_element_1.parse_Fembic(
                    contact_input, lineno, nodelist, materiallist, loadlist, nodetable
                );
            }
            ...
            if (! node[1].hasContact_LineElementConnectedTo(node[2])) {
                ...
                // Set the input
                internal_contact_line_element_2.parse_Fembic(
                    contact_input, lineno, nodelist, materiallist, loadlist, nodetable
                );
            }
            ...
            if (! node[2].hasContact_LineElementConnectedTo(node[3])) {
                ...
                // Set the input
                internal_contact_line_element_3.parse_Fembic(
                    contact_input, lineno, nodelist, materiallist, loadlist, nodetable
                );
            }
            ...
            if (! node[3].hasContact_LineElementConnectedTo(node[0])) {
                ...
                // Set the input
                internal_contact_line_element_4.parse_Fembic(
                    contact_input, lineno, nodelist, materiallist, loadlist, nodetable
                );
            }
        }
...
/**
        ...
        throws IllegalArgumentException
    {
        ...
        if (! T_is_set) {
            throw new IllegalArgumentException(
                "No Thickness (T) defined for Shell_BT_4 element nr" + number
            );
        }
    ...
    public void checkIfFailed() {
        ...
        if (material[0].failureStressIsSet()) {
            ...
            double s = 0.7071f * Math.sqrt(
                    Math.pow(
                        stress[printed_integration_point].get(0, 0) -
                        stress[printed_integration_point].get(1, 0), 2
                    ) +
                    Math.pow(
                        stress[printed_integration_point].get(1, 0) -
                        stress[printed_integration_point].get(2, 0), 2
                    ) +
                    Math.pow(
                        stress[printed_integration_point].get(2, 0) -
                        stress[printed_integration_point].get(0, 0), 2
                    ) +
                    (6 * (Math.pow(
                        stress[printed_integration_point].get(3, 0), 2
                    ) +
                    Math.pow(stress[printed_integration_point].get(4, 0), 2) +
                    Math.pow(stress[printed_integration_point].get(5, 0), 2)))
                );
        ...
        if (material[0].failureStrainIsSet()) {
            ...
            double e = 0.4714f * Math.sqrt(
                    Math.pow(
                        strain[printed_integration_point].get(0, 0) -
                        strain[printed_integration_point].get(1, 0), 2
                    ) +
                    Math.pow(
                        strain[printed_integration_point].get(1, 0) -
                        strain[printed_integration_point].get(2, 0), 2
                    ) +
                    Math.pow(
                        strain[printed_integration_point].get(2, 0) -
                        strain[printed_integration_point].get(0, 0), 2
                    ) +
                    (1.5 * (Math.pow(
                        strain[printed_integration_point].get(3, 0), 2
                    ) +
                    Math.pow(strain[printed_integration_point].get(4, 0), 2) +
                    Math.pow(strain[printed_integration_point].get(5, 0), 2)))
                );
    ...
    public String print_Gid(int ctrl, int gpn) {
        ...
        case MESH_HEADER:
             ...
             * In this case, the type of element is Quadilateral and it uses 4 nodes
             */
            out = new String(
                    "MESH \"MeshType" + type +
        ...
        case RESULT_HEADER:
            ...
            /* Print the header of the result file, for the block of Quadrilateral elements.
        ...
        case RESULT_STRESS_LOCAL:
            ...
            if (gpn == 0) {
                out = new String(number + " "); // Element number must start the first gauss point results
        ...
        throws IllegalArgumentException
    {
        ...
        /* Make a local copy of the material object and keep it inside the element.
         * The reason for this is twofold.
         * 1. The material law is now tied to the element and will remember the element properties and history
         * 2. Since the material is cloned now, after Initialize, all the parameters are defined in the material object and are
         * now automatically transferred to the local copy.
         *
         * Note that one copy of the materiallaw is required for each intergration point in the element.
         */
        try {
        ...
        // Set up the index and weights of the integration points for a gaussian integration
        ...
        // Print by default the middle integration point if nothing else specified
        if (! PIP_is_set) {
     ...
     * to have the following node numbering y     3    I            2 I
     * I________ x     0                1
     */
    public void updateLocalCoordinateSystem() {
        this.calculateLocalBaseVectors(
            ...
            node[3].getX_pos(), node[3].getY_pos(), node[3].getZ_pos(),
    ...
    private void calculateLocalVariables() {
         ...
         * Normal formula works for small distorsions: area = (1/2)*(X31*Y42 + X24*Y31)
         * Note that for large deformations, the area will become very small as the element
         * reaches self contact. The area is then corrected and recalculated in the
         * calculateStrain() method if the ADVANCED contact option is used.
         */
        area = 0.5 * (((p.get(6, 0) - p.get(0, 0)) * (p.get(10, 0) -
            p.get(4, 0))) +
            ((p.get(3, 0) - p.get(9, 0)) * (p.get(7, 0) - p.get(1, 0))));
    ...
    /**
     * This method is called by any internal nodes that this element uses.
     * This is the case if advanced or advanced_edge contact options have been
     * used. The method is called by the internal node when the calculateNewPosition
...
public void setInternalNodePosition() {
    ...
    // Update the position of the local node if needed
        middle_node.setX_pos_orig(
            (node[0].getX_pos() + node[1].getX_pos() + node[2].getX_pos() +
            node[3].getX_pos()) / 4
        );
        middle_node.setY_pos_orig(
            (node[0].getY_pos() + node[1].getY_pos() + node[2].getY_pos() +
            node[3].getY_pos()) / 4
        );
        middle_node.setZ_pos_orig(
            (node[0].getZ_pos() + node[1].getZ_pos() + node[2].getZ_pos() +
            node[3].getZ_pos()) / 4
        );
}
...
public void deActivate() {
    super.deActivate();
    if (internal_contact_element_1 != null) internal_contact_element_1.deActivate();
    if (internal_contact_element_2 != null) internal_contact_element_2.deActivate();
    if (internal_contact_element_3 != null) internal_contact_element_3.deActivate();
    if (internal_contact_element_4 != null) internal_contact_element_4.deActivate();
    if (internal_contact_line_element_1 != null) internal_contact_line_element_1.deActivate();
    if (internal_contact_line_element_2 != null) internal_contact_line_element_2.deActivate();
    if (internal_contact_line_element_3 != null) internal_contact_line_element_3.deActivate();
    if (internal_contact_line_element_4 != null) internal_contact_line_element_4.deActivate();
    if (middle_node != null) middle_node.deActivate();
}
