...
public class Contact_Triangle extends Element {
    ...
    public synchronized void calculateContactForces() {
        ...
        while (! finished) {
            ...
            if (this.isInContact(current)) {
                ...
                // Add this contact force to the total contact force (for tracking)
                ...
                /* .. and add a reaction force to the three element nodes
                 *  The natural coordinate detemines how the force should be distributed.
                 *  The negative sign comes from the fact that it is a reaction force.
                 */
    ...
    private boolean isInContact(Node contact_node) {
        ...
        if (
            (contact_node == node[0]) || (contact_node == node[1]) ||
            (contact_node == node[2])
        ) {
            return false;
        }
        ...
        // Convert to natural coordinate system (assumes a position matrix [1,x,y])
        xsi1 = A00 + (A01 * pos.get(0, 0)) + (A02 * pos.get(1, 0));
        xsi2 = (A11 * pos.get(0, 0)) + (A12 * pos.get(1, 0));
        xsi3 = A22 * pos.get(1, 0);

        // Check if any of the natural coordinates are negative (means it is not inside the contact element borders)
        if (xsi1 < 0) {
            return false;
        }
        ...
        // All values are positive. This node may be in contact. Now, check the distance to the surface.
    ...
    private void addFriction(Node contact_node) {
        ...
        if (tmp.vec_is_set == false) {
            ...
            force.set(
                0, 0,
                force.get(0, 0) +
                ((((pos.get(2, 0) > 0) ? (-0.5) : 0.5) * friction_factor * force.get(
                    2, 0
                ) * tmp.vecX) / length1)
            );
            force.set(
        ...
        length2 = Math.sqrt(
                (v2.get(0, 0) * v2.get(0, 0)) + (v2.get(1, 0) * v2.get(1, 0))
            );

        // Calculate the angle between the vectors
        angle = Math.abs(
                Math.acos(
                    v1.transpose().times(v2).get(0, 0) / (length1 * length2)
                )
            );

        // Add the friction force (with a ramp up if angle > 90 degrees to prevent oscillation)
        force.set(
            0, 0,
            force.get(0, 0) +
            ((((pos.get(2, 0) > 0) ? (-1) : 1) * ((angle < 1.5708) ? 1.0 : 0.5) * friction_factor * force.get(
                2, 0
            ) * tmp.vecX) / length1)
        );
        force.set(
    ...
    /**
     * The update of the area is needed for the internal contact elements
     * used in Shell BT 4 for advanced_edge option. Otherwise, the middle
     * node has not been updated and the contact element area gets wrong.
     */
