...
public class Rod_2 extends Element {
     ...
     * coordinate system before adding to the node. Some simplifications has been
     * made though as seen later.
     * Creation date: (10/5/2001 3:32:57 PM)
     */
        ...
        throws IllegalArgumentException
    {
        ...
        initial_length = java.lang.Math.sqrt(
                ((node[0].getX_pos() - node[1].getX_pos()) * (node[0].getX_pos() -
                node[1].getX_pos())) +
                ((node[0].getY_pos() - node[1].getY_pos()) * (node[0].getY_pos() -
                node[1].getY_pos())) +
                ((node[0].getZ_pos() - node[1].getZ_pos()) * (node[0].getZ_pos() -
                node[1].getZ_pos()))
            );
         ...
         * 1. Rotational inertia around the center longitudinal axis (m*d2/8)
         * 2. Rotational inertia around the middle point of the bar. Any centripetal axis (m*l2/12).
         *
         * It has been determined that inertia applied to nodes has limited
         * influence and that some simplifications can be made.
         * In this case, the largest inertia will be kept and applied
         * to all directions. Couplings will be set to zero. This
         * simplification is also used in the other elements.
         * 2005-09-17 version 0.6.3 - JF  */

        inertia.set(0,0,Math.max((mass * diameter * diameter) / 8, (mass * initial_length * initial_length) / 12));
    ...
    /**
     * This is a biggie. The contact forces are calculated here. This is done
     * in the following way: Let this element check it's segment(s) against
     * any neighbouring nodes.     calculate distance from segment to node if
     * distance &lt; tolerance then  reactionforce = f(tolerance) add
     * reactionforce to node & element nodes             Continue this check
     * until all the nodes within the contact tolerance has been checked.
     * Creation date: (2001-10-26 01.06.35)
     */
     ...
     *
     * @exception java.lang.Error The exception description.
     */
    public void calculateStrain(double timestep, int integration_point)
    {
        ...
        // Calculate also the new cross section area (assuming incompressible material)
     ...
     * a matrix. Here, it is only one value, which makes things easier.
     * Creation date: (2001-10-23 13.51.34)
     *
     * @exception java.lang.Error The exception description.
     */
     ...
     * smallest of the current timestep and the element timestep since it is
     * the smallest element that determines the stability of the solution.
     * Creation date: (10/12/01 %T)
     *
    ...
    public double checkTimestep(double current_timestep) {
        ...
        //
        critical_length = java.lang.Math.sqrt(
                ((xpos2 - xpos1) * (xpos2 - xpos1)) +
                ((ypos2 - ypos1) * (ypos2 - ypos1)) +
                ((zpos2 - zpos1) * (zpos2 - zpos1))
            );

        /* Calculate critical timestep using the Courant condition
         *  timestep <= L/c
         *  L= element length
         *  c= aucustic wave speed = sqrt(E/density)
         */
        ...
        throws java.text.ParseException
    {
        ...
        if (Contact == BASIC) {
            ...
            if (Factor_is_set) i += 3;
            if (Friction_is_set) i += 3;
        ...
        throws IllegalArgumentException
    {
        ...
        if (! D_is_set) {
            throw new IllegalArgumentException("No Diameter defined for Rod element nr" + number);
        }

        if (! Nodes_are_set) {
            throw new IllegalArgumentException("No nodes defined for Rod element nr" + number);
        }

        if (! Material_is_set) {
            throw new IllegalArgumentException("No Material defined for Rod element nr" + number);
        }
    ...
    public String print_Gid(int ctrl, int gpn) {
        ...
        case MESH_HEADER:
             ...
             * In this case, the type of element is Linear and it a Rod uses 2 nodes
             */
            out = new String(
                    "MESH \"MeshType" + type +
        ...
        case RESULT_HEADER:
            ...
            /* Print the header of the result file, for the block of Rod elements */
            out = new String(
        ...
        case RESULT_STRESS_LOCAL:
            ...
            /* Print the Gauss stresses for this element, remembering the previous line index */
     ...
     * local x-axis will run from node 1 to node 2. The local y-axis will be
     * in the plane given by the local x-axis     and the point formed by the
     * coordinates of node 1, but with and offset of 1 mm in x and y
     * direction.     The local z-axis will be orthogonal to the same plane.
     * In the case of a rod, the directions of the y-and z-axis are really not
     * that important since it is symmetric.     The local system is unified
     * in length. The forces and inertias can now be expresses relative to
     * this     system and then transformed Creation date: (19/11/01 %T)
     */
    public void updateLocalCoordinateSystem() {
        this.calculateLocalBaseVectors(
            0, 0, 0, node[1].getX_pos() - node[0].getX_pos(),
            node[1].getY_pos() - node[0].getY_pos(),
            node[1].getZ_pos() - node[0].getZ_pos(),
            node[1].getZ_pos() - node[0].getZ_pos(),
            node[0].getX_pos() - node[1].getX_pos(),
            node[1].getY_pos() - node[0].getY_pos(), local_coordinate_system
        );
