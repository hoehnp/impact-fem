...
public class ModelSmp implements Runnable, ExceptionListener {
    ...
    /**
     * This method initializes the problem in the following steps: 
     * 1. Create a file object so that the indata file can be read. 
     * 2. Question the file object of all the variables etc that is required to set up all the
     * memory variables etc correctly. 
        ...
        throws Exception 
    {
        ...
        // Create an indata file object that we can ask for the things we require
        ...
        try {
            ...
            for (i = 0; i < number_of_nodes; i++) {
                ...
                temporary_node = indatafile.getNextNode(constraintlist, loadlist);
                                
                nodelist.addElement(temporary_node);
                nodetable.put(new Integer(temporary_node.getNumber()), temporary_node);
            ...
            for (i = 0; i < number_of_elements; i++) {
                elementlist.addElement(indatafile.getNextElement(materiallist, nodelist, loadlist, nodetable));
            ...
            for (i = 0; i < number_of_trackers; i++) {
                trackerlist.addElement(
                    indatafile.getNextTracker(nodelist, elementlist)
                );
            }
            ...
            // Create an outdata file that will print the results in the format we wish
            resultwriter = indatafile.getWriter(nodelist, elementlist, controlset, null);

            // Create a trackwriter that will print all trackresults in right format
            trackwriter = indatafile.getTrackWriter(trackerlist, controlset, null);
        } catch (java.text.ParseException e) {
            // If we end up here, something is wrong in the indata file, reading the parameters
    ...
    /**
     * This method prints the requested state of the solution to a file The
     * file will be called the same name as the indatafile, but with an ending
     * of *.res 
     */
    private void print() {
        System.out.println();
    ...
    public void setInitialConditions() {
        ...
        /*
         * Set up the neighbour pointers needed for contact algorithm
         * Give each node a handle to it's neighbour
         */
        ...
        // Now, if autostep is used, loop through all elements and determine the smallest timestep
        ...
        if (controlset.getTimestep(0) == 0) {
            ...
            for (i = 0; i < number_of_elements; i++) {
                timestep = ((Element) elementlist.elementAt(i)).checkTimestep(timestep);
            }
        } else {
    ...
    public void solve() throws InterruptedException {
    	...
    	System.out.println("Distributing the model on " + nr_of_CPUs + " CPU:s");
   		for (int i=0; i<elementlist.size(); i++) {
   			temporary_element = (Element)elementlist.elementAt(i);
   			worker[temporary_element.getCpu_number()].addElement(temporary_element);
   		...
   		for(i=0; i<nr_of_CPUs; i++) {
        	System.out.println("CPU "+i+" Nodes: "+worker_node[i]+" \tElements: "+worker_element[i]);
        }
        ...
        // This is the main loop which will run in tandem with the other threads;
        for(time = time; time <= controlset.getEndtime(); time += timestep){
        	...
        	for (i = 0; i < nodelist.size(); i++) {
        	    ...
        	    if (!temporary_node.isDeActivated()) {
        	        // Update acceleration, velocity and position of node (supply timestep because it is needed in the calculations)
        	        // Note, the update includes application of boundary conditions.
        	        // The update also includes addition of any external forces from boundary conditions.
        	...
        	if (controlset.getTimestep(time) == 0) {
        		...
        		if(timestep>timestep_error) timestep = timestep_error;
        		autostep = false;    		
        	}
        	...
        	if (controlset.timeToPrint(time)) {
        		...
        		time_remained = (((time_info_tmp - time_info) * ((int) ((controlset.getEndtime() -
    				time) / controlset.getPrintstep()))) / 1000);
        		time_h = (int) (time_remained / 3600);
        		time_m = (int) ((time_remained / 60) - (time_h * 60));
        		time_s = (int) (time_remained -
    				(((int) (time_remained / 60)) * 60));
        		time_str = "";
        		...
        		System.out.println("Time: " + time + "\tRemaining time (hh:mm:ss) " + time_str);
    ...
    /**
     * This is where we end up when a serious exception has occurred in the worker threads
     * 
     */
    ...
    public void writeAutoSave(){
    	if(controlset.getRestoreSaveStatus()==Controlset.RESTORE_SAVE_OFF) return;
    	try{
    		ObjectOutputStream aout = new ObjectOutputStream(new FileOutputStream(filename+".autosave"));
    		writeObject(aout);
    		aout.flush();
    		aout.close();
    		if(controlset.getRestoreSaveStatus()==Controlset.RESTORE_SAVE_PREVIOUS){
    			...
    			FileChannel fain = new FileInputStream(filename+".autosave").getChannel();
    			FileChannel faout = new FileOutputStream(filename+".autosave."+time).getChannel();
    			fain.transferTo(0, fain.size(), faout);
    			fain.close();
    			faout.close();    		
    			FileChannel frin = new FileInputStream(filename+".flavia.res").getChannel();
    			FileChannel frout = new FileOutputStream(filename+".flavia.res."+time).getChannel();
    			frin.transferTo(0, frin.size(), frout);
    			frin.close();
    			frout.close();
    			for (i = 0; i < number_of_trackers; i++) {
    				temporary_tracker = (Tracker) trackerlist.elementAt(i);
    				FileChannel ftin = new FileInputStream(temporary_tracker.filename).getChannel();
    				FileChannel ftout = new FileOutputStream(temporary_tracker.filename+"."+time).getChannel();
    ...
    public void readAutoSave(){
    	...
    	try{
    		ObjectInputStream ain = new ObjectInputStream(new FileInputStream(filename+".autosave"));
    		readObject(ain);
    		ain.close();
    		System.out.println("Found saved result for Time: " + time + "\tTime step: "+timestep); 
    	} catch (Exception e) {if(! (e instanceof FileNotFoundException)) e.printStackTrace();	}
    }
    ...
    /*
     * At occurrence of an error calculation stops. 
     ...
     * 
     * При возникновении ошибки расчет останавливается. 
     * В файл сохранены время ошибки, шаг ошибки. 
     * При повторном запуске модели шаг времени уменьшается в два раза 
     * ( timestep = timestep_error / 2 ). 
     * Так происходит каждый раз при запуске модели. 
     * Это позволяет правильно определить шаг.  
     */
    ...
    public void readAutoSaveError(){
    	...
    	try{
    		...
    		System.out.println("Found saved error for Time: " + time + "\tTime step: "+timestep_error);
        	timestep_error /= 2.0d; 
        	if(timestep>timestep_error) timestep = timestep_error;
