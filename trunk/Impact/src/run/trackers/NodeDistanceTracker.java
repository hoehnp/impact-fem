...
public class NodeDistanceTracker extends Tracker {
	...
	private void collectOwnData() throws IllegalArgumentException {
		...
		result = Math
				.sqrt(((node1.getX_pos() - node2.getX_pos()) * (node1
						.getX_pos() - node2.getX_pos()))
						+ ((node1.getY_pos() - node2.getY_pos()) * (node1
								.getY_pos() - node2.getY_pos()))
						+ ((node1.getZ_pos() - node2.getZ_pos()) * (node1
								.getZ_pos() - node2.getZ_pos())));
	}
		...
		while (i < param.length) {
			...
			if (param[i].getw().toUpperCase().equals("NODES")
				...
				try {
					...
					node1 = super.findNode(super.getNodeNumber(1, param[i + 2]
							.getw().toUpperCase()), globalnodelist);
					node2 = super.findNode(super.getNodeNumber(2, param[i + 2]
							.getw().toUpperCase()), globalnodelist);
	...
	public void checkIndata() throws IllegalArgumentException {
		if (!Nodes_are_set) {
			throw new IllegalArgumentException(
					"No nodes defined for this node distance tracker: "
							+ number);
		}
	...
	public void print_Gid(int ctrl, double currtime) throws IOException,
		...
		case RESULT_HEADER:
			...
			try {
				...
				System.out
						.println("Error in creating the NodeDistanceTracker file: "
								+ filename);
				throw ioe;
			}
		...
		case RESULT:
			...
			try {
				...
				System.out
						.println("Error in writing results to the NodeDistanceTracker file: "
								+ filename);
				throw ioe;
			}
