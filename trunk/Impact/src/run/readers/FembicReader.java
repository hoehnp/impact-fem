...
public class FembicReader extends Reader {
    ...
    private java.lang.String[] keywords = {
        "TITLE", "CONTROLS", "ELEMENTS", "NODES", "LOADS", "CONSTRAINTS",
        "MATERIALS", "TRACKERS", "GROUPS", "GEOMETRY"
    };
    ...
    public void close() {
        try {
            ...
            System.out.println(
                "An IOException has occurred when closing the indata file"
            );
        ...
        try {
            if (! control_in_block) {
                while (str.nextToken() != java.io.StreamTokenizer.TT_EOF) {
                    if (str.ttype == java.io.StreamTokenizer.TT_WORD) {
                        if (
                            str.sval.toUpperCase().equals("CONTROLS") &&
            ...
            if (! control_in_block) {
                throw new java.text.ParseException(
                    "No controls block found or missing controls ", str.lineno()
                );
            }

            // Time to read the next control set
            // Assume that we are at the beginning of a line, so the first token should now be the control command
            ...
            while (str.nextToken() != str.TT_EOF) {
                if (str.ttype == str.TT_WORD) {
                    if (! this.isAKeyword(str.sval)) {
                        // Go ahead and read this line. Feed it to the controlset.
                        str.pushBack();
                        temporary_controlset.parse_Fembic(
                            tokenize(str), str.lineno()
                        );
                    } else {
                        str.pushBack();
            ...
            // OK, everything read now. Lets end. The data has already been filled into the controlset given as an argument
     ...
     * varability inside the definition of the element. Therefore, a new
     * method called parse_Fembic is designed for each element where the
     * checking and parsing of the variables are taken care of. A beauty of
     * object oriented design is that the parent class is returned  from this
     * method since the functionality really is irrelevant to the program from
     * here on. Creation date: (26/08/01 %T)
     *
        ...
        throws java.text.ParseException
    {
        ...
        try {
            ...
            while (str.nextToken() != java.io.StreamTokenizer.TT_NUMBER) {
                ...
                //This is not the case, so search until the next block of elements is found
                ...
                while (str.nextToken() != java.io.StreamTokenizer.TT_EOF) {
                    if (str.ttype == java.io.StreamTokenizer.TT_WORD) {
                        if (str.sval.toUpperCase().equals("ELEMENTS")) {
                            if (
                                str.nextToken() == java.io.StreamTokenizer.TT_WORD
                            ) {
            ...
            if (! element_in_block) {
                throw new java.text.ParseException(
                    "No elements block found or missing elements", str.lineno()
                );
            }

            // Parse the element number and put into the node object
            temporary_element = Element.getElementOfType_Fembic(
                    current_element_type
                );
            temporary_element.setNumber((int) str.nval);
            // Continue and read the element parameters, assuming the format 'param = val'
            temporary_element.parse_Fembic(
                tokenize(str), str.lineno(), nodelist, materiallist, loadlist, nodetable
            );

            // Check all required data has been filled in
            temporary_element.checkIndata();

            /* Getting this far means that all parameters for the element is defined,
               so return the temporary element now
               Note the upcasting!!*/
            ...
            throw new IllegalArgumentException(
                e.getMessage() + " in line: " + (e.getErrorOffset() - 1)
            );
        } catch (IllegalArgumentException e) {
     ...
     * varability inside the definition of the tracker. Therefore, a new
     * method called parse_Fembic is designed for each tracker where the
     * checking and parsing of the variables are taken care of. A beauty of
     * object oriented design is that the parent class is returned  from this
     * method since the functionality really is irrelevant to the program from
     * here on. Creation date: (26/08/01 %T)
        ...
        throws java.text.ParseException
    {
        ...
        try {
            ...
            while (str.nextToken() != java.io.StreamTokenizer.TT_NUMBER) {
                ...
                //This is not the case, so search until the next block of trackers is found
            ...
            if (! tracker_in_block) {
                throw new java.text.ParseException(
                    "No trackers block found or missing trackers", str.lineno()
                );
            }

            // Parse the tracker number and put into the tracker object
            temporary_tracker = Tracker.getTrackerOfType_Fembic(
                    current_tracker_type
                );
            temporary_tracker.setNumber((int) str.nval);
            // Continue and read the tracker parameters, assuming the format 'param = val'
            temporary_tracker.parse_Fembic(
                tokenize(str), str.lineno(), nodelist, elementlist
            );

            // Check all required data has been filled in
            temporary_tracker.checkIndata();

            /* Getting this far means that all parameters for the tracker is defined,
               so return the temporary tracker now
               Note the upcasting!!*/
        ...
        throws java.text.ParseException
    {
        ...
        try {
            ...
            // Parse the tracker number and put into the tracker object
            temporary_constraint = Constraint.getConstraintOfType_Fembic(
                    current_constraint_type
                );
            temporary_constraint.setName(str.sval.toUpperCase().trim());
            // Continue and read the tracker parameters, assuming the format 'param = val'
            temporary_constraint.parse_Fembic(
                tokenize(str), str.lineno(), nodelist
            );
    ...
    /**
     * This method reads the next load, sets up all the paremeters and returns
     * a handle to the load. Creation date: (26/08/01 %T)
     *
     * @return krockpackage.Node
     *
     * @exception java.text.ParseException The exception description.
     */
    public Load getNextLoad(RplVector nodelist)
        throws java.text.ParseException
    {
        // Assume the file has been opened
        ...
        try {
            if (! in_block) {
                while (str.nextToken() != java.io.StreamTokenizer.TT_EOF) {
                    if (str.ttype == java.io.StreamTokenizer.TT_WORD) {
                        if (
                            str.sval.toUpperCase().equals("LOADS") &&
                            (str.nextToken() == str.TT_EOL)
                        ) {
                            in_block = true;
    ...
    /**
     * This method reads the next node, sets up all the paremeters and returns
     * a handle to the node. Creation date: (26/08/01 %T)
     *
     * @return krockpackage.Node
     *
     * @exception java.text.ParseException The exception description.
        ...
        throws java.text.ParseException
    {
        ...
        try {
            ...
            while (
                (str.nextToken() != java.io.StreamTokenizer.TT_NUMBER) &&
                (str.ttype != java.io.StreamTokenizer.TT_EOF)
            ) {
                // This is not the case. Now search until next block of nodes is found and try again.
                in_block = false;
                str.pushBack();
                ...
                while (str.nextToken() != java.io.StreamTokenizer.TT_EOF) {
                    if (str.ttype == java.io.StreamTokenizer.TT_WORD) {
                        if (
                            str.sval.toUpperCase().equals("NODES") &&
                            (str.nextToken() == str.TT_EOL)
                        ) {
                            in_block = true;
            ...
            temporary_node.parse_Fembic(
                tokenize(str), str.lineno(), constraintlist, loadlist
            );
        ...
        throws java.text.ParseException
    {
        ...
        try {
            // Start examining the file and continue until end of file is reached
            while (str.nextToken() != str.TT_EOF) {
                ...
                if (str.ttype == str.TT_WORD) {
                    if (str.sval.toUpperCase().equals("CONSTRAINTS")) {
                        ...
                        while (str.nextToken() != str.TT_EOF) {
                            // Repeat this until a keyword is found. That means block is finished. 
                            if (str.ttype == str.TT_WORD) {
                                if (this.isAKeyword(str.sval)) {
                                    str.pushBack();
                            ...
                            /* getting this far means we are now about to read trackers
                               The first token should be a word if a tracker is defined */
                            if (str.ttype == str.TT_WORD) {
                                noc++;
            ...
            System.out.println(
                "IOException when reading a constraintline from indata file"
            );
            this.close();
        ...
        throws java.text.ParseException
    {
        ...
        try {
            ...
            while (str.nextToken() != str.TT_EOF) {
                ...
                if (str.ttype == str.TT_WORD) {
                    if (
                        ...
                        while (str.nextToken() != str.TT_EOF) {
                            // getting this far means we are now about to read constraints
                            // The first token should be a constraint name if an element is defined
                            if (str.ttype == str.TT_WORD) {
                                if (! this.isAKeyword(str.sval)) {
                                    noc++;
            ...
            System.out.println(
                "IOException when reading a line from indata file"
            );
            this.close();
                ...
                if (str.ttype == str.TT_WORD) {
                    if (str.sval.toUpperCase().equals("ELEMENTS")) {
                        ...
                        while (str.nextToken() != str.TT_EOF) {
                            ...
                            /* getting this far means we are now about to read elements
                               The first token should be a line number if an element is defined */
     ...
     * sectionmoment 1 nodes = [2,3] filename = .....         Creation date:
     * (21/08/01 %T)
     *
     * @return int
     */
        ...
        throws java.text.ParseException
    {
        ...
        try {
                ...
                if (str.ttype == str.TT_WORD) {
                    if (str.sval.toUpperCase().equals("TRACKERS")) {
                        ...
                        while (str.nextToken() != str.TT_EOF) {
                            ...
                            /* getting this far means we are now about to read trackers
                               The first token should be a line number if a tracker is defined */
                            if (str.ttype == str.TT_NUMBER) {
                                not++;
            ...
            System.out.println(
                "IOException when reading a trackerline from indata file"
            );
            this.close();
	    ...
	    throws java.text.ParseException
	{
	    ...
	    try {
	        ...
	        while (str.nextToken() != str.TT_EOF) {
	            ...
	            if (str.ttype == str.TT_WORD) {
	                if (
	                    str.sval.toUpperCase().equals("GROUPS") &&
	                    (str.nextToken() == str.TT_EOL)
	                ) {
	                    while (str.nextToken() != str.TT_EOF) {
	                        // getting this far means we are now about to read groups
	                        // The first token should be a group set name if a set is defined
	                        if (str.ttype == str.TT_WORD) {
	                            if (! this.isAKeyword(str.sval)) {
	                                noc++;
            ...
            while (str.nextToken() != str.TT_EOF) {
                ...
                if (str.ttype == str.TT_WORD) {
                    if (
                        ...
                        while (str.nextToken() != str.TT_EOF) {
                            // getting this far means we are now about to read loads
                            // The first token should be a load set name if a set is defined
                            if (str.ttype == str.TT_WORD) {
                                if (! this.isAKeyword(str.sval)) {
                                    noc++;
            ...
            while (str.nextToken() != str.TT_EOF) {
                ...
                if (str.ttype == str.TT_WORD) {
                    if (str.sval.toUpperCase().equals("MATERIALS")) {
                        ...
                        while (str.nextToken() != str.TT_EOF) {
                            // getting this far means we are now about to read elements
                            // The first token should be a material name if an element is defined
                            if (str.ttype == str.TT_WORD) {
                                if (! this.isAKeyword(str.sval)) {
                                    nom++;
                ...
                if (str.ttype == str.TT_WORD) {
                    if (
                        ...
                        while (str.nextToken() != str.TT_EOF) {
                            ...
                            // getting this far means we are now about to read nodes
                            // The first token should be a line number if an element is defined
                            if (str.ttype == str.TT_NUMBER) {
                                non++;
    ...
    public void open() {
        ...
        try {
            ...
            System.out.println(
                "An IllegalArgumentException has occurred when opening the indata file"
            );
    ...
    {
        Writer a;
        a = Writer.getWriterOfType_Fembic(
                control.getWriter(), nodelist, elementlist, cluster_nodes
            );
     ...
     */
    public TrackWriter getTrackWriter(RplVector trackerlist, Controlset control, RemoteObject[] cluster_nodes)
        throws RemoteException
    {
        TrackWriter a;
        a = TrackWriter.getTrackWriterOfType_Fembic(
        ...
        throws IOException
    {
        ...
        // Start by reading all the words and numbers on the line into the p vector.
        while (
        ...
        for (i = 0; i < v.size(); i++) {
            ...
            if (t.is_a_word()) {
                ...
                // This may be a beginning of a [ set. If so, search until next set if found.
                ...
                // Check to see if there is an = sign in there (i.e. no spaces around the =)
                // If so, split up into three tokens and make the parser read them again.
                if ((temp.indexOf("=") != -1) && (temp.length() > 1)) {
                    v.remove(i);

                    if (temp.indexOf("=") != 0) {
                        v.insertElementAt(
                            new Token(temp.substring(0, temp.indexOf("="))), i
                        );
                    } else {
                        i--;
                    }
