...
public class ModelCluster extends RemoteObject implements Runnable {
     ...
     * object of all the variables etc that is required to set up all the
     * memory variables etc correctly. 3. Set up the variables 4. Finished
     * Creation date: (2001-08-20 22:01:54)
     *
        ...
        throws Exception
    {
        ...
        try {
            ...
            //Fill the Elements array
				...
				for (i = 0; i < number_of_elements; i++) {
					elementlist.addElement(indatafile.getNextElement(
							materiallist, nodelist, loadlist, nodetable));
				}
            ...
            resultwriter = indatafile.getWriter(
                    nodelist, elementlist, controlset, cluster_nodes
                );

            // Create a trackwriter that will print all trackresults in right format
            trackwriter = indatafile.getTrackWriter(trackerlist, controlset, cluster_nodes);
        } catch (ParseException e) {
     ...
     * of .res Creation date: (2001-10-17 01.51.01)
     */
    private void print() {
        System.out.println();
        System.out.println();
    ...
    public void setInitialConditions() {
        ...
        */
        ...
        if (controlset.getTimestep(0) == 0) {
            ...
            timestep = 1E10;
            for (i = 0; i < number_of_elements; i++) {
                timestep = ((Element) elementlist.elementAt(i)).checkTimestep(
                        timestep
                    );
            }
        } else {
	...
	public void solve (Barrier barrier) throws Exception, InterruptedException {
		...
		System.out.println("Calculating " + elementindicies.length + " elements");
...
// 	Loop elements
			if (DEBUG) System.out.println("Client: " + client_nr + " Loop elements");
			for (i = 0; i < elementindicies.length; i++) {
			    
			    temporary_element = (Element) elementlist.elementAt(elementindicies[i]);
			    
			    if (!temporary_element.isDeActivated()) {
			        
			        // Determine the element local base vectors (local coordinate
			        // system)
			        temporary_element.updateLocalCoordinateSystem();
			        
			        // To reduce amount of memory in each element, loop these steps
			        // for each integration point
			        number_of_integration_points = temporary_element.getNumberOfIntegrationPoints();
			         ...
			         * integration points. Continue with the forces. By knowing all
			         * the strains in advance, the thickness reduction of shells can
...
// Remove failed elements
		if (DEBUG) System.out.println("Client: " + client_nr + " Failed elements");
    	if (failure_is_set) {
    		for (i = 0; i < elementindicies.length; i++) {
    			temporary_element = (Element) elementlist.elementAt(elementindicies[i]);
    			if (!temporary_element.isDeActivated()) {
...
// Loop constraints
		if (DEBUG) System.out.println("Client: " + client_nr + " Loop constraints");
			for (i = 0; i < number_of_constraints; i++) {
...
// Loop Trackers
			if (DEBUG) System.out.println("Client: " + client_nr + " Loop Trackers");
			for (i = 0; i < number_of_trackers; i++) {
...
// Update timestep
			if (DEBUG) System.out.println("Client: " + client_nr + " Update timestep");
...
// Check if it is time to print something
			if (DEBUG) System.out.println("Client: " + client_nr + " Print Results");
			if (controlset.timeToPrint(time)) {
					...
					System.out.println("Cluster node [" + client_nr + "] Time: " 
							+ ((time+"").length()>16?(time+"").substring(0,16):(time+""))
							+ "\tRemaining time (hh:mm:ss) " + time_str);
					time_info = time_info_tmp;
					
				try {
					resultwriter.writeParallel(filename, time, elementindicies, barrier, client_nr, nr_of_CPUs);
...
// Check if it is time to print something on the trackers
			if (DEBUG) System.out.println("Client: " + client_nr + " Print trackers");
...
// Clear nodal forces
			if (DEBUG) System.out.println("Client: " + client_nr + " Clear nodal forces");
...
// End loop			
	...
	/**
	 * This method calculates which objects are to be 
	 * calculated in this client process. Note that this 
	 * is calculated in parallel on all clients and should not
	 * give overlapping results is maximum efficiency is to be
	 * gained
	 */
...
//***********************************************************
		...
		for (int i=0; i<nodelist.size(); i++) {
			...
			if (tempnode.getCpu_number() == client_nr) nodeindicies[index++] = i;
		}
		...
		for (int i=0; i<elementlist.size(); i++) {
			...
			if (tempelement.getCpu_number() == client_nr) index++;
		}
		...
		for (int i=0; i<elementlist.size(); i++) {
			tempelement = (Element)elementlist.elementAt(i);
			if (tempelement.getCpu_number() == client_nr) elementindicies[index++] = i;
		}

	}

	/**
	 *  This method changes the reference on the node objects to use the 
	 * shared data in the shareddata object.
	 */
	...
	/**
	 *  This method makes any possible optimizations to the cluster setup
	 *  such as minimization of data transferred etc.
	 */
     ...
     * Each instance of this class will be run in a separate thread.
     * When started, this method will be run and most errors (exceptions) will be
     * migrated here.
     */
    public void run() {
