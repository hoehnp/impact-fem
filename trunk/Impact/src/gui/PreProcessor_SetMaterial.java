...
public class PreProcessor_SetMaterial extends JPanel {
  ...
  private JTable MatTable = new JTable(MatModel){
    ...
    public TableCellEditor getCellEditor(int row, int column) {
      if(column == 1) {
        Color cl = JColorChooser.showDialog(this,"Pick a Color",((Material)db.get(getValueAt(row,0))).color);
        if(cl!=null)((Material)db.get(getValueAt(row,0))).color=cl;
      }
  ...
  void b_set_actionPerformed(ActionEvent e) {
    Material mat = (Material)db.get(MatTable.getValueAt(MatTable.getSelectedRow(),0));
    if(mat==null) return;

    _Object[] obj = J3D.getSelectedObjects3D();
    for(int i=0; i<obj.length; i++){
      if(obj[i] instanceof _Element8){ ((_Element8)obj[i]).material=mat; }
      else if(obj[i] instanceof _Element3){ ((_Element3)obj[i]).material=mat; }
      else if(obj[i] instanceof _Element4){ ((_Element4)obj[i]).material=mat; }
      else if(obj[i] instanceof _Element2){ ((_Element2)obj[i]).material=mat; }
    }
    J3D.tree_reset();
    J3D.view_reset();
  }
