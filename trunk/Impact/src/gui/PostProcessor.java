...
public class PostProcessor extends JPanel {
  ...
  double maxdx,maxdy,maxdz,maxdi,maxex,maxey,maxez,maxexy,maxeyz,maxexz,maxsx,maxsy,maxsz,maxsxy,maxsyz,maxsxz,maxei,maxsi,
  ...
  private Canvas3D J3D =  new Canvas3D(SimpleUniverse.getPreferredConfiguration());
  ...
  private BoundingSphere bounds = new BoundingSphere(new Point3d(0.0,0.0,0.0), 1000000.0);
  ...
  private void jbInit() throws Exception {
	    ...
	    DirectionalLight lgt1 = new DirectionalLight(new Color3f(0.7f, 0.7f, 0.7f), new Vector3f(1.0f, 1.0f, -1.0f));
	    ...
	    clip_panel.sl_x.addChangeListener(new ChangeListener() {
	        public void stateChanged(ChangeEvent e) {
	            Vector4d eqn = new Vector4d(clip_panel.cb_x_reverse.isSelected()?1:-1.0, 0.0, 0.0, (clip_panel.cb_x_reverse.isSelected()?-1:1)*clip_panel.sl_x.getValue());
	            model_clip.setPlane(0, eqn);
	        }
	    });
	    clip_panel.sl_y.addChangeListener(new ChangeListener() {
	        public void stateChanged(ChangeEvent e) {
	            Vector4d eqn = new Vector4d(0.0, clip_panel.cb_y_reverse.isSelected()?1:-1.0, 0.0, (clip_panel.cb_y_reverse.isSelected()?-1:1)*clip_panel.sl_y.getValue());
	            model_clip.setPlane(1, eqn);
	        }
	    });
	    clip_panel.sl_z.addChangeListener(new ChangeListener() {
	        public void stateChanged(ChangeEvent e) {
	            Vector4d eqn = new Vector4d(0.0, 0.0, clip_panel.cb_z_reverse.isSelected()?1:-1.0, (clip_panel.cb_z_reverse.isSelected()?-1:1)*clip_panel.sl_z.getValue());
	            model_clip.setPlane(2, eqn);
	        }
	    });	    	    
	    clip_panel.cb_x_reverse.addItemListener(new java.awt.event.ItemListener() {
	        public void itemStateChanged(ItemEvent e) {
	        	Vector4d eqn = new Vector4d(clip_panel.cb_x_reverse.isSelected()?1:-1.0, 0.0, 0.0, (clip_panel.cb_x_reverse.isSelected()?-1:1)*clip_panel.sl_x.getValue());
	            model_clip.setPlane(0, eqn);
	        }
	    });
	    clip_panel.cb_y_reverse.addItemListener(new java.awt.event.ItemListener() {
	        public void itemStateChanged(ItemEvent e) {
	        	Vector4d eqn = new Vector4d(0.0, clip_panel.cb_y_reverse.isSelected()?1:-1.0, 0.0, (clip_panel.cb_y_reverse.isSelected()?-1:1)*clip_panel.sl_y.getValue());
	            model_clip.setPlane(1, eqn);
	        }
	    });
	    clip_panel.cb_z_reverse.addItemListener(new java.awt.event.ItemListener() {
	        public void itemStateChanged(ItemEvent e) {
	        	Vector4d eqn = new Vector4d(0.0, 0.0, clip_panel.cb_z_reverse.isSelected()?1:-1.0, (clip_panel.cb_z_reverse.isSelected()?-1:1)*clip_panel.sl_z.getValue());
	            model_clip.setPlane(2, eqn);
	        }
  ...
  public synchronized void load(String st){
    try{
      ...
      in.close();
      clip_panel.sl_x.setMinimum((int)(minx+mindx-Math.abs(minx*0.2)));
	  clip_panel.sl_x.setMaximum((int)(maxx+maxdx+Math.abs(maxx*0.2)));
	  clip_panel.sl_x.setValue((int)(minx+mindx-Math.abs(minx*0.2)));
	  clip_panel.sl_y.setMinimum((int)(miny+mindy-Math.abs(miny*0.2)));
	  clip_panel.sl_y.setMaximum((int)(maxy+maxdy+Math.abs(maxy*0.2)));
	  clip_panel.sl_y.setValue((int)(miny+mindy-Math.abs(miny*0.2)));
	  clip_panel.sl_z.setMinimum((int)(minz+mindz-Math.abs(minz*0.2)));
	  clip_panel.sl_z.setMaximum((int)(maxz+maxdz+Math.abs(maxz*0.2)));
	  clip_panel.sl_z.setValue((int)(minz+mindz-Math.abs(minz*0.2)));
	  model_clip.setInfluencingBounds(bounds);
	  System.out.print("... OK\n");
      headerMessage(null);
      try{
...
//    Set initial state    
  ...
  public Point3d toPoint3d(String st)throws Exception{
    ...
    st_t.nextToken();
    return new Point3d(new Float(st_t.nextToken()).floatValue(),new Float(st_t.nextToken()).floatValue(),new Float(st_t.nextToken()).floatValue());
  }
  ...
  public void draw(){
	  ...
	  if(cb_node.isSelected()){
		  ...
		  for(Enumeration en = db_node_tmp.keys(); en.hasMoreElements();){
			  ...
			  resultGroup.addChild(new PostProcessor_shp_oriented_text(key,new Point3f((float)(p1.x+p2.x*kd),(float)(p1.y+p2.y*kd),(float)(p1.z+p2.z*kd)), fontsize));
		  }
	  }
    ...
    if(cb_element.isSelected()){
      ...
      for(Enumeration en = db_stresses_tmp.keys(); en.hasMoreElements();){
        ...
        y/=arr.length;
        z/=arr.length;
        resultGroup.addChild(new PostProcessor_shp_oriented_text(key,new Point3f((float)x,(float)y,(float)z), fontsize));
      }
    }
        ...
        if(arr.length==2){
          ...
          if(cb_result.getSelectedIndex()==0){
        	if(this.cb_conturfill.isSelected()){
        		vLinesColor.addElement(getGradient(0,(float)Math.sqrt(p21.x*p21.x+p21.y*p21.y+p21.z*p21.z),(float)maxdi));
        		vLinesColor.addElement(getGradient(0,(float)Math.sqrt(p22.x*p22.x+p22.y*p22.y+p22.z*p22.z),(float)maxdi));
        	} else {
          	...
          	if(this.cb_conturfill.isSelected()){
        		vLinesColor.addElement(getGradient((float)mindx,(float)p21.x,(float)maxdx));
        		vLinesColor.addElement(getGradient((float)mindx,(float)p22.x,(float)maxdx));
        	} else {
        		vLinesColor.addElement(Color.black);
        		vLinesColor.addElement(Color.black);
        	}
        	...
        	if(this.cb_conturfill.isSelected()){
          		vLinesColor.addElement(getGradient((float)mindy,(float)p21.y,(float)maxdy));
          		vLinesColor.addElement(getGradient((float)mindy,(float)p22.y,(float)maxdy));
          	} else {
          		vLinesColor.addElement(Color.black);
          		vLinesColor.addElement(Color.black);
          	}
        	...
        	if(this.cb_conturfill.isSelected()){
            	vLinesColor.addElement(getGradient((float)mindz,(float)p21.z,(float)maxdz));
            	vLinesColor.addElement(getGradient((float)mindz,(float)p22.z,(float)maxdz));
            } else {
            	vLinesColor.addElement(Color.black);
            	vLinesColor.addElement(Color.black);
            } 
          ...
          if(this.cb_mesh.isSelected()){
        	  ...
        	  vLines.addElement(new Point3d(x3,y3,z3));
        	  vLines.addElement(new Point3d(x1,y1,z1));
        	  for(int i=0; i<6; i++)vLinesColor.addElement(Color.black);
          }
          if(this.cb_conturfill.isSelected()){
        	  if(cb_result.getSelectedIndex()==0){
        		  ...
        		  vTriangleColor.addElement(getGradient(0,(float)Math.sqrt(p21.x*p21.x+p21.y*p21.y+p21.z*p21.z),(float)maxdi));
        		  vTriangleColor.addElement(getGradient(0,(float)Math.sqrt(p22.x*p22.x+p22.y*p22.y+p22.z*p22.z),(float)maxdi));
        		  vTriangleColor.addElement(getGradient(0,(float)Math.sqrt(p23.x*p23.x+p23.y*p23.y+p23.z*p23.z),(float)maxdi));
        	  }else if(cb_result.getSelectedIndex()==1){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  vTriangleColor.addElement(getGradient((float)mindx,(float)p21.x,(float)maxdx));
        		  vTriangleColor.addElement(getGradient((float)mindx,(float)p22.x,(float)maxdx));
        		  vTriangleColor.addElement(getGradient((float)mindx,(float)p23.x,(float)maxdx));
        	  }else if(cb_result.getSelectedIndex()==2){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  vTriangleColor.addElement(getGradient((float)mindy,(float)p21.y,(float)maxdy));
        		  vTriangleColor.addElement(getGradient((float)mindy,(float)p22.y,(float)maxdy));
        		  vTriangleColor.addElement(getGradient((float)mindy,(float)p23.y,(float)maxdy));
        	  }else if(cb_result.getSelectedIndex()==3){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  vTriangleColor.addElement(getGradient((float)mindz,(float)p21.z,(float)maxdz));
        		  vTriangleColor.addElement(getGradient((float)mindz,(float)p22.z,(float)maxdz));
        		  vTriangleColor.addElement(getGradient((float)mindz,(float)p23.z,(float)maxdz));
        	  }
          }
          ...
          if(this.cb_mesh.isSelected()){
        	  ...
        	  vLines.addElement(new Point3d(x4,y4,z4));
        	  vLines.addElement(new Point3d(x1,y1,z1));
        	  for(int i=0; i<8; i++)vLinesColor.addElement(Color.black);
          }
          if(this.cb_conturfill.isSelected()){
        	  if(cb_result.getSelectedIndex()==0){
        		  ...
        		  vQuadColor.addElement(getGradient(0,(float)Math.sqrt(p21.x*p21.x+p21.y*p21.y+p21.z*p21.z),(float)maxdi));
        		  vQuadColor.addElement(getGradient(0,(float)Math.sqrt(p22.x*p22.x+p22.y*p22.y+p22.z*p22.z),(float)maxdi));
        		  vQuadColor.addElement(getGradient(0,(float)Math.sqrt(p23.x*p23.x+p23.y*p23.y+p23.z*p23.z),(float)maxdi));
        		  vQuadColor.addElement(getGradient(0,(float)Math.sqrt(p24.x*p24.x+p24.y*p24.y+p24.z*p24.z),(float)maxdi));
        	  }else if(cb_result.getSelectedIndex()==1){
        		  vQuad.addElement(new Point3d(x1,y1,z1));
        		  vQuad.addElement(new Point3d(x2,y2,z2));
        		  vQuad.addElement(new Point3d(x3,y3,z3));
        		  vQuad.addElement(new Point3d(x4,y4,z4));
        		  vQuadColor.addElement(getGradient((float)mindx,(float)p21.x,(float)maxdx));
        		  vQuadColor.addElement(getGradient((float)mindx,(float)p22.x,(float)maxdx));
        		  vQuadColor.addElement(getGradient((float)mindx,(float)p23.x,(float)maxdx));
        		  vQuadColor.addElement(getGradient((float)mindx,(float)p24.x,(float)maxdx));
        	  }else if(cb_result.getSelectedIndex()==2){
        		  vQuad.addElement(new Point3d(x1,y1,z1));
        		  vQuad.addElement(new Point3d(x2,y2,z2));
        		  vQuad.addElement(new Point3d(x3,y3,z3));
        		  vQuad.addElement(new Point3d(x4,y4,z4));
        		  vQuadColor.addElement(getGradient((float)mindy,(float)p21.y,(float)maxdy));
        		  vQuadColor.addElement(getGradient((float)mindy,(float)p22.y,(float)maxdy));
        		  vQuadColor.addElement(getGradient((float)mindy,(float)p23.y,(float)maxdy));
        		  vQuadColor.addElement(getGradient((float)mindy,(float)p24.y,(float)maxdy));
        	  }else if(cb_result.getSelectedIndex()==3){
        		  vQuad.addElement(new Point3d(x1,y1,z1));
        		  vQuad.addElement(new Point3d(x2,y2,z2));
        		  vQuad.addElement(new Point3d(x3,y3,z3));
        		  vQuad.addElement(new Point3d(x4,y4,z4));
        		  vQuadColor.addElement(getGradient((float)mindz,(float)p21.z,(float)maxdz));
        		  vQuadColor.addElement(getGradient((float)mindz,(float)p22.z,(float)maxdz));
        		  vQuadColor.addElement(getGradient((float)mindz,(float)p23.z,(float)maxdz));
        		  vQuadColor.addElement(getGradient((float)mindz,(float)p24.z,(float)maxdz));
        	  }
          }
          ...
          if(this.cb_mesh.isSelected()){
        	  ...
        	  vLines.addElement(new Point3d(x8,y8,z8));
        	  vLines.addElement(new Point3d(x4,y4,z4));
        	  for(int i=0; i<24; i++)vLinesColor.addElement(Color.black);
          }
          if(this.cb_conturfill.isSelected()){
        	  if(cb_result.getSelectedIndex()==0){
        		  vQuad.addElement(new Point3d(x1,y1,z1));
        		  ...
        		  vQuadColor.addElement(getGradient(0,(float)Math.sqrt(p25.x*p25.x+p25.y*p25.y+p25.z*p25.z),(float)maxdi));
        		  vQuadColor.addElement(getGradient(0,(float)Math.sqrt(p26.x*p26.x+p26.y*p26.y+p26.z*p26.z),(float)maxdi));
        		  vQuadColor.addElement(getGradient(0,(float)Math.sqrt(p27.x*p27.x+p27.y*p27.y+p27.z*p27.z),(float)maxdi));
        		  vQuadColor.addElement(getGradient(0,(float)Math.sqrt(p28.x*p28.x+p28.y*p28.y+p28.z*p28.z),(float)maxdi));
        		  ...
        		  vQuadColor.addElement(getGradient((float)mindx,(float)p25.x,(float)maxdx));
        		  vQuadColor.addElement(getGradient((float)mindx,(float)p26.x,(float)maxdx));
        		  vQuadColor.addElement(getGradient((float)mindx,(float)p27.x,(float)maxdx));
        		  vQuadColor.addElement(getGradient((float)mindx,(float)p28.x,(float)maxdx));
        		  ...
        		  vQuadColor.addElement(getGradient((float)mindy,(float)p25.y,(float)maxdy));
        		  vQuadColor.addElement(getGradient((float)mindy,(float)p26.y,(float)maxdy));
        		  vQuadColor.addElement(getGradient((float)mindy,(float)p27.y,(float)maxdy));
        		  vQuadColor.addElement(getGradient((float)mindy,(float)p28.y,(float)maxdy));
        		  ...
        		  vQuadColor.addElement(getGradient((float)mindz,(float)p25.z,(float)maxdz));
        		  vQuadColor.addElement(getGradient((float)mindz,(float)p26.z,(float)maxdz));
        		  vQuadColor.addElement(getGradient((float)mindz,(float)p27.z,(float)maxdz));
        		  vQuadColor.addElement(getGradient((float)mindz,(float)p28.z,(float)maxdz));
        ...
        if(arr.length==2){
          ...
          vLines.addElement(new Point3d(x1,y1,z1));
      	  ...
      	  if(cb_result.getSelectedIndex()==4 || cb_result.getSelectedIndex()==5){
      		  if(this.cb_conturfill.isSelected()){
      			  vLinesColor.addElement(getGradient((float)minei,ee[0],(float)maxei));
      			  vLinesColor.addElement(getGradient((float)minei,ee[0],(float)maxei));
      		  } else {
      			  vLinesColor.addElement(Color.black);
      			  vLinesColor.addElement(Color.black);
          ...
          if(this.cb_conturfill.isSelected()){
        	  if(cb_result.getSelectedIndex()==4){
        		  ...
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  float ei = (float)(Math.sqrt(2)/3*Math.sqrt(Math.pow(ee[0]-ee[1],2)+Math.pow(ee[1]-ee[2],2)+Math.pow(ee[2]-ee[0],2)+1.5*(ee[3]*ee[3]+ee[4]*ee[4]+ee[5]*ee[5])));
        		  Color fill = getGradient((float)minei,ei,(float)maxei);
        		  for(int i=0; i<3; i++)vTriangleColor.addElement(fill);
        	  }else if(cb_result.getSelectedIndex()==5){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  Color fill = getGradient((float)minex,ee[0],(float)maxex);
        		  for(int i=0; i<3; i++)vTriangleColor.addElement(fill);
        	  }else if(cb_result.getSelectedIndex()==6){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  Color fill = getGradient((float)miney,ee[1],(float)maxey);
        		  for(int i=0; i<3; i++)vTriangleColor.addElement(fill);
        	  }else if(cb_result.getSelectedIndex()==7){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  Color fill = getGradient((float)minez,ee[2],(float)maxez);
        		  for(int i=0; i<3; i++)vTriangleColor.addElement(fill);
        	  }else if(cb_result.getSelectedIndex()==8){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  Color fill = getGradient((float)minexy,ee[3],(float)maxexy);
        		  for(int i=0; i<3; i++)vTriangleColor.addElement(fill);
        	  }else if(cb_result.getSelectedIndex()==9){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  Color fill = getGradient((float)mineyz,ee[4],(float)maxeyz);
        		  for(int i=0; i<3; i++)vTriangleColor.addElement(fill);
        	  }else if(cb_result.getSelectedIndex()==10){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  Color fill = getGradient((float)minexz,ee[5],(float)maxexz);
        		  for(int i=0; i<3; i++)vTriangleColor.addElement(fill);
        	  }
          }
        }else if(arr.length==4){
          ...
          if(this.cb_conturfill.isSelected()){
        	  if(cb_result.getSelectedIndex()==4){
        		  ...
        		  float ei = (float)(Math.sqrt(2)/3*Math.sqrt(Math.pow(ee[0]-ee[1],2)+Math.pow(ee[1]-ee[2],2)+Math.pow(ee[2]-ee[0],2)+1.5*(ee[3]*ee[3]+ee[4]*ee[4]+ee[5]*ee[5])));
        		  Color fill = getGradient((float)minei,ei,(float)maxei);
        		  for(int i=0; i<4; i++)vQuadColor.addElement(fill);
        	  }else if(cb_result.getSelectedIndex()==5){
        		  vQuad.addElement(new Point3d(x1,y1,z1));
        		  vQuad.addElement(new Point3d(x2,y2,z2));
        		  vQuad.addElement(new Point3d(x3,y3,z3));
          ...
          if(this.cb_conturfill.isSelected()){
        	  if(cb_result.getSelectedIndex()==4){
        		  if(ee.length==6){
            		  ...
            		  for(int i=0; i<24; i++)vQuadColor.addElement(fill);
        		  }else{
        			  float ei1 = (float)(Math.sqrt(2)/3*Math.sqrt(Math.pow(ee[0]-ee[1],2)+Math.pow(ee[1]-ee[2],2)+Math.pow(ee[2]-ee[0],2)+1.5*(ee[3]*ee[3]+ee[4]*ee[4]+ee[5]*ee[5])));
        			  float ei2 = (float)(Math.sqrt(2)/3*Math.sqrt(Math.pow(ee[6]-ee[7],2)+Math.pow(ee[7]-ee[8],2)+Math.pow(ee[8]-ee[6],2)+1.5*(ee[9]*ee[9]+ee[10]*ee[10]+ee[11]*ee[11])));
        			  float ei3 = (float)(Math.sqrt(2)/3*Math.sqrt(Math.pow(ee[12]-ee[13],2)+Math.pow(ee[13]-ee[14],2)+Math.pow(ee[14]-ee[12],2)+1.5*(ee[15]*ee[15]+ee[16]*ee[16]+ee[17]*ee[17])));
        			  float ei4 = (float)(Math.sqrt(2)/3*Math.sqrt(Math.pow(ee[18]-ee[19],2)+Math.pow(ee[19]-ee[20],2)+Math.pow(ee[20]-ee[18],2)+1.5*(ee[21]*ee[21]+ee[22]*ee[22]+ee[23]*ee[23])));
        			  float ei5 = (float)(Math.sqrt(2)/3*Math.sqrt(Math.pow(ee[24]-ee[25],2)+Math.pow(ee[25]-ee[26],2)+Math.pow(ee[26]-ee[24],2)+1.5*(ee[27]*ee[27]+ee[28]*ee[28]+ee[29]*ee[29])));
        			  float ei6 = (float)(Math.sqrt(2)/3*Math.sqrt(Math.pow(ee[30]-ee[31],2)+Math.pow(ee[31]-ee[32],2)+Math.pow(ee[32]-ee[30],2)+1.5*(ee[33]*ee[33]+ee[34]*ee[34]+ee[35]*ee[35])));
        			  float ei7 = (float)(Math.sqrt(2)/3*Math.sqrt(Math.pow(ee[36]-ee[37],2)+Math.pow(ee[37]-ee[38],2)+Math.pow(ee[38]-ee[36],2)+1.5*(ee[39]*ee[39]+ee[40]*ee[40]+ee[41]*ee[41])));
        			  float ei8 = (float)(Math.sqrt(2)/3*Math.sqrt(Math.pow(ee[42]-ee[43],2)+Math.pow(ee[43]-ee[44],2)+Math.pow(ee[44]-ee[42],2)+1.5*(ee[45]*ee[45]+ee[46]*ee[46]+ee[47]*ee[47])));
        		  ...
        		  if(ee.length==6){
        			  ...
        			  Color fill1 = getGradient((float)minex,ee[0],(float)maxex);
        			  Color fill2 = getGradient((float)minex,ee[6],(float)maxex);
        			  Color fill3 = getGradient((float)minex,ee[12],(float)maxex);
        			  Color fill4 = getGradient((float)minex,ee[18],(float)maxex);
        			  Color fill5 = getGradient((float)minex,ee[24],(float)maxex);
        			  Color fill6 = getGradient((float)minex,ee[30],(float)maxex);
        			  Color fill7 = getGradient((float)minex,ee[36],(float)maxex);
        			  Color fill8 = getGradient((float)minex,ee[42],(float)maxex);
        			  vQuad.addElement(new Point3d(x1,y1,z1));
        			  vQuad.addElement(new Point3d(x2,y2,z2));
        			  vQuad.addElement(new Point3d(x3,y3,z3));
        			  vQuad.addElement(new Point3d(x4,y4,z4));
        		  ...
        		  if(ee.length==6){
        			  ...
        			  Color fill1 = getGradient((float)miney,ee[1],(float)maxey);
        			  Color fill2 = getGradient((float)miney,ee[7],(float)maxey);
        			  Color fill3 = getGradient((float)miney,ee[13],(float)maxey);
        			  Color fill4 = getGradient((float)miney,ee[19],(float)maxey);
        			  Color fill5 = getGradient((float)miney,ee[25],(float)maxey);
        			  Color fill6 = getGradient((float)miney,ee[31],(float)maxey);
        			  Color fill7 = getGradient((float)miney,ee[37],(float)maxey);
        			  Color fill8 = getGradient((float)miney,ee[43],(float)maxey);
        			  vQuad.addElement(new Point3d(x1,y1,z1));
        			  vQuad.addElement(new Point3d(x2,y2,z2));
        			  vQuad.addElement(new Point3d(x3,y3,z3));
        			  vQuad.addElement(new Point3d(x4,y4,z4));
        		  ...
        		  if(ee.length==6){
        			  ...
        			  Color fill1 = getGradient((float)minez,ee[2],(float)maxez);
        			  Color fill2 = getGradient((float)minez,ee[8],(float)maxez);
        			  Color fill3 = getGradient((float)minez,ee[14],(float)maxez);
        			  Color fill4 = getGradient((float)minez,ee[20],(float)maxez);
        			  Color fill5 = getGradient((float)minez,ee[26],(float)maxez);
        			  Color fill6 = getGradient((float)minez,ee[32],(float)maxez);
        			  Color fill7 = getGradient((float)minez,ee[38],(float)maxez);
        			  Color fill8 = getGradient((float)minez,ee[44],(float)maxez);
        			  vQuad.addElement(new Point3d(x1,y1,z1));
        			  vQuad.addElement(new Point3d(x2,y2,z2));
        			  vQuad.addElement(new Point3d(x3,y3,z3));
        			  vQuad.addElement(new Point3d(x4,y4,z4));
        		  ...
        		  if(ee.length==6){
        			  ...
        			  Color fill1 = getGradient((float)minexy,ee[3],(float)maxexy);
        			  Color fill2 = getGradient((float)minexy,ee[9],(float)maxexy);
        			  Color fill3 = getGradient((float)minexy,ee[15],(float)maxexy);
        			  Color fill4 = getGradient((float)minexy,ee[21],(float)maxexy);
        			  Color fill5 = getGradient((float)minexy,ee[27],(float)maxexy);
        			  Color fill6 = getGradient((float)minexy,ee[33],(float)maxexy);
        			  Color fill7 = getGradient((float)minexy,ee[39],(float)maxexy);
        			  Color fill8 = getGradient((float)minexy,ee[45],(float)maxexy);
        			  vQuad.addElement(new Point3d(x1,y1,z1));
        			  vQuad.addElement(new Point3d(x2,y2,z2));
        			  vQuad.addElement(new Point3d(x3,y3,z3));
        			  vQuad.addElement(new Point3d(x4,y4,z4));
        		  ...
        		  if(ee.length==6){
        			  ...
        			  Color fill1 = getGradient((float)mineyz,ee[4],(float)maxeyz);
        			  Color fill2 = getGradient((float)mineyz,ee[10],(float)maxeyz);
        			  Color fill3 = getGradient((float)mineyz,ee[16],(float)maxeyz);
        			  Color fill4 = getGradient((float)mineyz,ee[22],(float)maxeyz);
        			  Color fill5 = getGradient((float)mineyz,ee[28],(float)maxeyz);
        			  Color fill6 = getGradient((float)mineyz,ee[34],(float)maxeyz);
        			  Color fill7 = getGradient((float)mineyz,ee[40],(float)maxeyz);
        			  Color fill8 = getGradient((float)mineyz,ee[46],(float)maxeyz);
        			  vQuad.addElement(new Point3d(x1,y1,z1));
        			  vQuad.addElement(new Point3d(x2,y2,z2));
        			  vQuad.addElement(new Point3d(x3,y3,z3));
        			  vQuad.addElement(new Point3d(x4,y4,z4));
        		  ...
        		  if(ee.length==6){
        			  ...
        			  Color fill1 = getGradient((float)minexz,ee[5],(float)maxexz);
        			  Color fill2 = getGradient((float)minexz,ee[11],(float)maxexz);
        			  Color fill3 = getGradient((float)minexz,ee[17],(float)maxexz);
        			  Color fill4 = getGradient((float)minexz,ee[23],(float)maxexz);
        			  Color fill5 = getGradient((float)minexz,ee[29],(float)maxexz);
        			  Color fill6 = getGradient((float)minexz,ee[35],(float)maxexz);
        			  Color fill7 = getGradient((float)minexz,ee[41],(float)maxexz);
        			  Color fill8 = getGradient((float)minexz,ee[47],(float)maxexz);
        			  vQuad.addElement(new Point3d(x1,y1,z1));
        			  vQuad.addElement(new Point3d(x2,y2,z2));
        			  vQuad.addElement(new Point3d(x3,y3,z3));
        			  vQuad.addElement(new Point3d(x4,y4,z4));
      ...
      for(Enumeration en = db_stresses_tmp.keys(); en.hasMoreElements();){
        ...
        if(arr.length==2){
        	...
        	if(cb_result.getSelectedIndex()==11 || cb_result.getSelectedIndex()==12){
        		if(this.cb_conturfill.isSelected()){
        			vLinesColor.addElement(getGradient((float)minsi,ee[0],(float)maxsi));
        			vLinesColor.addElement(getGradient((float)minsi,ee[0],(float)maxsi));
        		} else {
        			vLinesColor.addElement(Color.black);
        			vLinesColor.addElement(Color.black);
          ...
          if(this.cb_conturfill.isSelected()){
        	  if(cb_result.getSelectedIndex()==11){
        		  ...
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  float ei = (float)(Math.sqrt(2)/2*Math.sqrt(Math.pow(ee[0]-ee[1],2)+Math.pow(ee[1]-ee[2],2)+Math.pow(ee[2]-ee[0],2)+6*(ee[3]*ee[3]+ee[4]*ee[4]+ee[5]*ee[5])));
        		  Color fill = getGradient((float)minsi,ei,(float)maxsi);
        		  for(int i=0; i<3; i++)vTriangleColor.addElement(fill);
        	  }else if(cb_result.getSelectedIndex()==12){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  Color fill = getGradient((float)minsx,ee[0],(float)maxsx);
        		  for(int i=0; i<3; i++)vTriangleColor.addElement(fill);
        	  }else if(cb_result.getSelectedIndex()==13){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  Color fill = getGradient((float)minsy,ee[1],(float)maxsy);
        		  for(int i=0; i<3; i++)vTriangleColor.addElement(fill);
        	  }else if(cb_result.getSelectedIndex()==14){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  Color fill = getGradient((float)minsz,ee[2],(float)maxsz);
        		  for(int i=0; i<3; i++)vTriangleColor.addElement(fill);
        	  }else if(cb_result.getSelectedIndex()==15){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  Color fill = getGradient((float)minsxy,ee[3],(float)maxsxy);
        		  for(int i=0; i<3; i++)vTriangleColor.addElement(fill);
        	  }else if(cb_result.getSelectedIndex()==16){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  Color fill = getGradient((float)minsyz,ee[4],(float)maxsyz);
        		  for(int i=0; i<3; i++)vTriangleColor.addElement(fill);
        	  }else if(cb_result.getSelectedIndex()==17){
        		  vTriangle.addElement(new Point3d(x1,y1,z1));
        		  vTriangle.addElement(new Point3d(x2,y2,z2));
        		  vTriangle.addElement(new Point3d(x3,y3,z3));
        		  Color fill = getGradient((float)minsxz,ee[5],(float)maxsxz);
        		  for(int i=0; i<3; i++)vTriangleColor.addElement(fill);
        	  }
          }
        }else if(arr.length==4){
          ...
          if(this.cb_conturfill.isSelected()){
        	  if(cb_result.getSelectedIndex()==11){
        		  if(ee.length==6){
        			  ...
        			  float ei1 = (float)(Math.sqrt(2)/2*Math.sqrt(Math.pow(ee[0]-ee[1],2)+Math.pow(ee[1]-ee[2],2)+Math.pow(ee[2]-ee[0],2)+6*(ee[3]*ee[3]+ee[4]*ee[4]+ee[5]*ee[5])));
        			  float ei2 = (float)(Math.sqrt(2)/2*Math.sqrt(Math.pow(ee[6]-ee[7],2)+Math.pow(ee[7]-ee[8],2)+Math.pow(ee[8]-ee[6],2)+6*(ee[9]*ee[9]+ee[10]*ee[10]+ee[11]*ee[11])));
        			  float ei3 = (float)(Math.sqrt(2)/2*Math.sqrt(Math.pow(ee[12]-ee[13],2)+Math.pow(ee[13]-ee[14],2)+Math.pow(ee[14]-ee[12],2)+6*(ee[15]*ee[15]+ee[16]*ee[16]+ee[17]*ee[17])));
        			  float ei4 = (float)(Math.sqrt(2)/2*Math.sqrt(Math.pow(ee[18]-ee[19],2)+Math.pow(ee[19]-ee[20],2)+Math.pow(ee[20]-ee[18],2)+6*(ee[21]*ee[21]+ee[22]*ee[22]+ee[23]*ee[23])));
        			  float ei5 = (float)(Math.sqrt(2)/2*Math.sqrt(Math.pow(ee[24]-ee[25],2)+Math.pow(ee[25]-ee[26],2)+Math.pow(ee[26]-ee[24],2)+6*(ee[27]*ee[27]+ee[28]*ee[28]+ee[29]*ee[29])));
        			  float ei6 = (float)(Math.sqrt(2)/2*Math.sqrt(Math.pow(ee[30]-ee[31],2)+Math.pow(ee[31]-ee[32],2)+Math.pow(ee[32]-ee[30],2)+6*(ee[33]*ee[33]+ee[34]*ee[34]+ee[35]*ee[35])));
        			  float ei7 = (float)(Math.sqrt(2)/2*Math.sqrt(Math.pow(ee[36]-ee[37],2)+Math.pow(ee[37]-ee[38],2)+Math.pow(ee[38]-ee[36],2)+6*(ee[39]*ee[39]+ee[40]*ee[40]+ee[41]*ee[41])));
        			  float ei8 = (float)(Math.sqrt(2)/2*Math.sqrt(Math.pow(ee[42]-ee[43],2)+Math.pow(ee[43]-ee[44],2)+Math.pow(ee[44]-ee[42],2)+6*(ee[45]*ee[45]+ee[46]*ee[46]+ee[47]*ee[47])));
        		  ...
        		  if(ee.length==6){
        			  ...
        			  Color fill1 = getGradient((float)minsx,ee[0],(float)maxsx);
        			  Color fill2 = getGradient((float)minsx,ee[6],(float)maxsx);
        			  Color fill3 = getGradient((float)minsx,ee[12],(float)maxsx);
        			  Color fill4 = getGradient((float)minsx,ee[18],(float)maxsx);
        			  Color fill5 = getGradient((float)minsx,ee[24],(float)maxsx);
        			  Color fill6 = getGradient((float)minsx,ee[30],(float)maxsx);
        			  Color fill7 = getGradient((float)minsx,ee[36],(float)maxsx);
        			  Color fill8 = getGradient((float)minsx,ee[42],(float)maxsx);
        			  vQuad.addElement(new Point3d(x1,y1,z1));
        			  vQuad.addElement(new Point3d(x2,y2,z2));
        			  vQuad.addElement(new Point3d(x3,y3,z3));
        			  vQuad.addElement(new Point3d(x4,y4,z4));
        		  ...
        		  if(ee.length==6){
        			  ...
        			  Color fill1 = getGradient((float)minsy,ee[1],(float)maxsy);
        			  Color fill2 = getGradient((float)minsy,ee[7],(float)maxsy);
        			  Color fill3 = getGradient((float)minsy,ee[13],(float)maxsy);
        			  Color fill4 = getGradient((float)minsy,ee[19],(float)maxsy);
        			  Color fill5 = getGradient((float)minsy,ee[25],(float)maxsy);
        			  Color fill6 = getGradient((float)minsy,ee[31],(float)maxsy);
        			  Color fill7 = getGradient((float)minsy,ee[37],(float)maxsy);
        			  Color fill8 = getGradient((float)minsy,ee[43],(float)maxsy);
        			  vQuad.addElement(new Point3d(x1,y1,z1));
        			  vQuad.addElement(new Point3d(x2,y2,z2));
        			  vQuad.addElement(new Point3d(x3,y3,z3));
        			  vQuad.addElement(new Point3d(x4,y4,z4));
        		  ...
        		  if(ee.length==6){
        			  ...
        			  Color fill1 = getGradient((float)minsz,ee[2],(float)maxsz);
        			  Color fill2 = getGradient((float)minsz,ee[8],(float)maxsz);
        			  Color fill3 = getGradient((float)minsz,ee[14],(float)maxsz);
        			  Color fill4 = getGradient((float)minsz,ee[20],(float)maxsz);
        			  Color fill5 = getGradient((float)minsz,ee[26],(float)maxsz);
        			  Color fill6 = getGradient((float)minsz,ee[32],(float)maxsz);
        			  Color fill7 = getGradient((float)minsz,ee[38],(float)maxsz);
        			  Color fill8 = getGradient((float)minsz,ee[44],(float)maxsz);
        			  vQuad.addElement(new Point3d(x1,y1,z1));
        			  vQuad.addElement(new Point3d(x2,y2,z2));
        			  vQuad.addElement(new Point3d(x3,y3,z3));
        			  vQuad.addElement(new Point3d(x4,y4,z4));
        		  ...
        		  if(ee.length==6){
        			  ...
        			  Color fill1 = getGradient((float)minsxy,ee[3],(float)maxsxy);
        			  Color fill2 = getGradient((float)minsxy,ee[9],(float)maxsxy);
        			  Color fill3 = getGradient((float)minsxy,ee[15],(float)maxsxy);
        			  Color fill4 = getGradient((float)minsxy,ee[21],(float)maxsxy);
        			  Color fill5 = getGradient((float)minsxy,ee[27],(float)maxsxy);
        			  Color fill6 = getGradient((float)minsxy,ee[33],(float)maxsxy);
        			  Color fill7 = getGradient((float)minsxy,ee[39],(float)maxsxy);
        			  Color fill8 = getGradient((float)minsxy,ee[45],(float)maxsxy);
        			  vQuad.addElement(new Point3d(x1,y1,z1));
        			  vQuad.addElement(new Point3d(x2,y2,z2));
        			  vQuad.addElement(new Point3d(x3,y3,z3));
        			  vQuad.addElement(new Point3d(x4,y4,z4));
        		  ...
        		  if(ee.length==6){
        			  ...
        			  Color fill1 = getGradient((float)minsyz,ee[4],(float)maxsyz);
        			  Color fill2 = getGradient((float)minsyz,ee[10],(float)maxsyz);
        			  Color fill3 = getGradient((float)minsyz,ee[16],(float)maxsyz);
        			  Color fill4 = getGradient((float)minsyz,ee[22],(float)maxsyz);
        			  Color fill5 = getGradient((float)minsyz,ee[28],(float)maxsyz);
        			  Color fill6 = getGradient((float)minsyz,ee[34],(float)maxsyz);
        			  Color fill7 = getGradient((float)minsyz,ee[40],(float)maxsyz);
        			  Color fill8 = getGradient((float)minsyz,ee[46],(float)maxsyz);
        			  vQuad.addElement(new Point3d(x1,y1,z1));
        			  vQuad.addElement(new Point3d(x2,y2,z2));
        			  vQuad.addElement(new Point3d(x3,y3,z3));
        			  vQuad.addElement(new Point3d(x4,y4,z4));
        		  ...
        		  if(ee.length==6){
        			  ...
        			  Color fill1 = getGradient((float)minsxz,ee[5],(float)maxsxz);
        			  Color fill2 = getGradient((float)minsxz,ee[11],(float)maxsxz);
        			  Color fill3 = getGradient((float)minsxz,ee[17],(float)maxsxz);
        			  Color fill4 = getGradient((float)minsxz,ee[23],(float)maxsxz);
        			  Color fill5 = getGradient((float)minsxz,ee[29],(float)maxsxz);
        			  Color fill6 = getGradient((float)minsxz,ee[35],(float)maxsxz);
        			  Color fill7 = getGradient((float)minsxz,ee[41],(float)maxsxz);
        			  Color fill8 = getGradient((float)minsxz,ee[47],(float)maxsxz);
        			  vQuad.addElement(new Point3d(x1,y1,z1));
        			  vQuad.addElement(new Point3d(x2,y2,z2));
        			  vQuad.addElement(new Point3d(x3,y3,z3));
        			  vQuad.addElement(new Point3d(x4,y4,z4));
    ...
    if(cb_gradientresult.isSelected()){
      ...
      for(int i=0; i<17; i++) {
    	  ...
    	  if(st_val.length()>len){
    		  if(st_val.toUpperCase().indexOf("E")!=-1){
    			  String st2 = st_val.substring(st_val.toUpperCase().indexOf("E"),st_val.length());
      ...
      bgNode.setImage(new ImageComponent2D(ImageComponent2D.FORMAT_RGB8,image));
    }else{ bgNode.setImage(null);}
    ...
    if(vLines.size()>0){
    	...
    	LineArray gla = new LineArray(MPoint.length, LineArray.COORDINATES | LineArray.COLOR_3);
    ...
    if(vTriangle.size()>0){
    	...
    	TriangleArray gta = new TriangleArray(MPoint.length, TriangleArray.COORDINATES | TriangleArray.COLOR_3);
    ...
    if(vQuad.size()>0){
    	...
    	QuadArray gqa = new QuadArray(MPoint.length, TriangleArray.COORDINATES | TriangleArray.COLOR_3);
          ...
          if(choise == JFileChooser.APPROVE_OPTION && fd.getSelectedFile()!=null){
              ...
              //if (fd.getFileFilter().equals(f1)) {
                  ...
                  for(int i=0; i<time_step.getModel().getSize(); i++){
                      ...
                      String resultFilename = file+File.separator+time_step.getSelectedValue()+".png"; 
...
public void headerMessage(String m) {
	...
	public void save_Image(String file) {
	    ...
	    try {
	      ...
	      try { J3D.waitForOffScreenRendering(); }catch(Exception e) {}
	      GraphicsContext3D  ctx = J3D.getGraphicsContext3D();
	      int w = J3D.getWidth();
	      int h = J3D.getHeight();
	      BufferedImage image = new BufferedImage(w, h, BufferedImage.TYPE_INT_RGB);
	      ImageComponent2D image2d = new ImageComponent2D(ImageComponent.FORMAT_RGB, image);
	      javax.media.j3d.Raster ras = new javax.media.j3d.Raster(new Point3f( -1.0f, -1.0f, -1.0f ),  javax.media.j3d.Raster.RASTER_COLOR, 0, 0, w, h, image2d, null );
	      ctx.flush(true);
	      ctx.readRaster( ras );
	      png = new PngEncoderB(ras.getImage().getImage(),false,0,5); // Encode at level 5 compression
	...
	public void view_zoom_all(){
		...
		tr3D_new.setScale(1/Math.max(maxx-minx, Math.max(maxy-miny,maxz-minz)));
		trGroup.setTransform(tr3D_new);
	}
