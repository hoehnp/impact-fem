...
public class Processor extends JPanel implements ExceptionListener {
  ...
  ImageIcon img_solid = new ImageIcon(Processor.class.getResource("solid.gif"));
  ...
  ImageIcon img_zooma = new ImageIcon(Processor.class.getResource("zooma.gif"));
  ImageIcon img_zoomin = new ImageIcon(Processor.class.getResource("zoomin.gif"));
  ImageIcon img_zoomout = new ImageIcon(Processor.class.getResource("zoomout.gif"));
  ImageIcon img_center = new ImageIcon(PreProcessor.class.getResource("center.png"));
  ImageIcon img_view_top = new ImageIcon(Processor.class.getResource("view_top.gif"));
  ImageIcon img_view_bottom = new ImageIcon(Processor.class.getResource("view_bottom.gif"));
  ImageIcon img_view_left = new ImageIcon(Processor.class.getResource("view_left.gif"));
  ImageIcon img_view_right = new ImageIcon(Processor.class.getResource("view_right.gif"));
  ImageIcon img_view_front = new ImageIcon(Processor.class.getResource("view_front.gif"));
  ImageIcon img_view_back = new ImageIcon(Processor.class.getResource("view_back.gif"));
  ImageIcon img_view_sw = new ImageIcon(Processor.class.getResource("view_sw.gif"));
  ImageIcon img_view_se = new ImageIcon(Processor.class.getResource("view_se.gif"));
  ImageIcon img_view_ne = new ImageIcon(Processor.class.getResource("view_ne.gif"));
  ImageIcon img_view_nw = new ImageIcon(Processor.class.getResource("view_nw.gif"));
  ImageIcon img_config = new ImageIcon(Processor.class.getResource("config.gif"));
  Thread[] tsolve;
  Thread tend;
  ...
  private TableModel tableModel = new AbstractTableModel() {
    ...
    public int getRowCount() { return table_materials.length; }
    public Object getValueAt(int row, int col){ return table_materials[row][col]+"";}
    public String getColumnName(int col) {return table_hdr_materials[col];}
...
//                        jToolBar2.addSeparator();
  ...
  /**
   * This method start Java Micro Benchmark  http://opt.sourceforge.net/
   * Java Micro Benchmark - control tasks required to determine the comparative 
   * performance characteristics of the computer system on different platforms. 
   * Can be used to guide optimization decisions and to compare different Java implementations.
   * 
   */
  
  void b_stats_actionPerformed() {
	  try {
          Desktop.getDesktop().open(new File(this.getClass().getResource("JavaMicroBenchmark.jnlp").toURI()));
      } catch (Exception e) {
          try { Desktop.getDesktop().browse( new URI("http://opt.sourceforge.net/")); } catch (Exception e1) {}
      }
  }
  ...
  public void show_model() {
          ...
          if(st.equals("NODES")){
              try{
                  while (stt.hasMoreTokens()) {
                      ...
                      nd = new _Node(Float.parseFloat(stt1.nextToken()),Float.parseFloat(stt1.nextToken()),Float.parseFloat(stt1.nextToken()));
                      db_node.put(key,nd);
                      if(st.indexOf("CONSTRAINT =")!=-1){
                          String st1 = st.substring(st.indexOf("CONSTRAINT")+10);
                          stt1 = new StringTokenizer(st1," \t");
                          stt1.nextToken();
                          nd.constraint = new Constraints(stt1.nextToken(),Color.BLUE);
                      }
      ...
      while (stt.hasMoreTokens()) {
          ...
          if(st.startsWith("ELEMENTS")){
            boolean is_Solid_Iso_4 = (st.toUpperCase().indexOf("SOLID_ISO_4")!=-1);
              try{
                  while (stt.hasMoreTokens()) {
                      st = stt.nextToken().trim().toUpperCase();
                      if(st.startsWith("#") || st.length()==0 || st.startsWith("ELEMENTS")){
                        if(st.startsWith("ELEMENTS")) is_Solid_Iso_4 = (st.toUpperCase().indexOf("SOLID_ISO_4")!=-1);
                        continue;
                      }
                      ...
                      int[] arr = new int[stt2.countTokens()+(is_Solid_Iso_4?1:0)];
                      int i=0;
                      ...
                      if(st_load.toUpperCase().indexOf("LOAD")!=-1){
                          st_load=st_load.substring(st_load.toUpperCase().indexOf("LOAD"));
  ...
  for(Enumeration en = db_element.keys(); en.hasMoreElements() ;) {
      ...
      if(arr.length==2){
          ...
          if (db_element_loads.get(key) != null)
              el2.load = new Loads((String)db_element_loads.get(key),Color.GREEN);
          J3D.add3D(el2);
          el2.set_Id(key);
          ...
          if (db_element_loads.get(key) != null)
              el3.load = new Loads((String)db_element_loads.get(key),Color.GREEN);
          J3D.add3D(el3);
          el3.set_Id(key);
          ...
          _Element4 el4 = new _Element4(p1,p2,p3,p4,Canvas3D.MESH_Shell_BT_4);
          if (db_element_loads.get(key) != null)
              el4.load = new Loads((String)db_element_loads.get(key),Color.GREEN);
          J3D.add3D(el4);
          el4.set_Id(key);
          ...
          _Node p3 = (_Node)db_node.get(arr[2]+"");
          _Node p4 = (_Node)db_node.get(arr[3]+"");
          _Element4 el4 = new _Element4(p1,p2,p3,p4,Canvas3D.MESH_Solid_Iso_4);
          if (db_element_loads.get(key) != null)
              el4.load = new Loads((String)db_element_loads.get(key),Color.GREEN);
          J3D.add3D(el4);
          el4.set_Id(key);
          ...
          if (db_element_loads.get(key) != null)
              el8.load = new Loads((String)db_element_loads.get(key),Color.GREEN);
          J3D.add3D(el8);
          el8.set_Id(key);
      }
  }
  ...
  void b_config_actionPerformed() {
    ...
    p.add(cb_trackers);
    p.add(cb_materials);
    JOptionPane.showMessageDialog(this,p,"Show",JOptionPane.PLAIN_MESSAGE,null);
    show_model();
  }
  ...
  void b_find_actionPerformed(ActionEvent e) {
    if(dialog==null)dialog = new JDialog(getFrame(this));
  ...
  private void stopThreads() {
	  for (int i=0 ; i < tsolve.length; i++)
          if (tsolve[i] != null) tsolve[i].interrupt();
      
	  if (tend != null) tend.interrupt();
  }
